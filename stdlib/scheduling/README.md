...
- `WithLock(locker, key)` — multi-replica de-duplication via a minimal local
  `Locker` interface. A `stdlib/lock.Locker` is adapted by the integration
...
Import path: `go-spring.org/stdlib/scheduling`.
...
`stdlib/lock.Locker` and bakes TTL / renew options into the local `Locker`
