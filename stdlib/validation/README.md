...
validation, split from its implementations the same way `stdlib/resilience`
and `stdlib/discovery` are. It answers "is this struct well-formed?" for both
...
- `ValidationErrors.Localize(msg func(key, args...) string)` renders per-field
  messages through any lookup function — typically bound to `stdlib/i18n`
...
Import path: `go-spring.org/stdlib/validation`.
