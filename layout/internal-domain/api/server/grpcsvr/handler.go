// Package grpcsvr wires gRPC handlers into a single controller and registers it
// as the pb.GS_PROJECT_NAMEServer bean the server adapter depends on.
package grpcsvr
...
func init() {
	// Export the composed controller as the generated gRPC server interface so
	// the server adapter can register it against grpc.Server in Run().
	gs.Provide(&GS_PROJECT_NAMEController{}).Export(gs.As[pb.GS_PROJECT_NAMEServer]())
}

// GS_PROJECT_NAMEController composes the per-domain gRPC controllers into a
// single value that satisfies pb.GS_PROJECT_NAMEServer. The embedded
...
type GS_PROJECT_NAMEController struct {
	orderCtrl.GrpcOrderController
	userCtrl.GrpcUserController
}
