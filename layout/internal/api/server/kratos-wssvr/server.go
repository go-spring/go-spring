// Package kratos_wssvr bootstraps a dedicated kratos-flavoured WebSocket server
// and binds it to the Go-Spring server lifecycle. kratos does not have a
// first-party WebSocket transport, so this package owns its own http.Server on
// an independent port (${spring.kratos.ws.server}) instead of sharing the core
// gs.HttpServeMux, mirroring the wssvr own-listener pattern. The
...
import (
	...
	"GS_PROJECT_MODULE/internal/api/server/kratos-wssvr/middleware"
...
func init() {
	// Register the kratos-WebSocket server and bind it to the Go-Spring server
	// lifecycle. Config is filled from ${spring.kratos.ws.server}; the server
	...
	gs.Provide(NewServer, gs.IndexArg(0, gs.TagArg("${spring.kratos.ws.server}"))).
		Export(gs.As[gs.Server]()).
		Condition(gs.OnBean[*GS_PROJECT_NAMEWebsocketController]())
}

// Config defines the kratos-WebSocket server configuration, bound from
// ${spring.kratos.ws.server}. Addr is the independent listen address; the
...
// share a port with the kratos HTTP server or the plain HTTP server.
type Server struct {
	cfg      Config
...
// NewServer creates a WebSocket Server from ${spring.kratos.ws.server} config,
