// Package goframehttpsvr wires goframe HTTP handlers into a single controller
// and registers it as the pb.GS_PROJECT_NAMEServer bean the server adapter
// depends on.
package goframehttpsvr
...
func init() {
	// Export the composed controller as the generated goframe HTTP server
...
// GS_PROJECT_NAMEController composes the per-domain goframe HTTP controllers
...
type GS_PROJECT_NAMEController struct {
	orderCtrl.GoframeHttpOrderController
	userCtrl.GoframeHttpUserController
}
