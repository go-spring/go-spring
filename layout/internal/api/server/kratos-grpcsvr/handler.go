// Package kratosgrpcsvr wires kratos-gRPC handlers into a single controller and
// registers it as the pb.GS_PROJECT_NAMEServer bean the server adapter depends
// on.
package kratos_grpcsvr
...
func init() {
	// Export the composed controller as the generated kratos-gRPC server
...
// GS_PROJECT_NAMEController composes the per-domain kratos-gRPC controllers
...
type GS_PROJECT_NAMEController struct {
	orderCtrl.KratosGrpcOrderController
	userCtrl.KratosGrpcUserController
}
