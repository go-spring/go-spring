// Package kitex_thriftsvr bootstraps the Kitex/Thrift server and binds it to the
// Go-Spring server lifecycle. It mirrors httpsvr but adapts a Kitex server to
// gs.Server so the container drives startup and graceful shutdown.
package kitex_thriftsvr

import (
	"context"

	svc "GS_PROJECT_MODULE/idl/kitex-thrift/kitex_gen/svc"
	"GS_PROJECT_MODULE/internal/api/server/kitex-thriftsvr/middleware"

	"go-spring.org/spring/gs"
)

func init() {
	// Register the Kitex/Thrift server and bind it to the Go-Spring server
	// lifecycle. Config is filled from the ${spring.kitex.thrift.server} prefix;
	// the server only materializes when a svc.GS_PROJECT_NAMEService bean exists,
	// mirroring the dubbo/grpc starters that gate on their generated service
	// interface.
	gs.Provide(NewServer, gs.IndexArg(0, gs.TagArg("${spring.kitex.thrift.server}"))).
		Export(gs.As[gs.Server]()).
		Condition(gs.OnBean[svc.GS_PROJECT_NAMEService]())
}

// Config defines Kitex/Thrift server configuration, bound from
// ${spring.kitex.thrift.server}.
type Config struct {
...
// Server adapts a Kitex/Thrift server to the Go-Spring server lifecycle. It
// owns a done channel so Run can park until Stop signals shutdown, matching
...
// NewServer creates a Kitex/Thrift Server from ${spring.kitex.thrift.server}
// config and the registered svc.GS_PROJECT_NAMEService bean.
...
// Run builds the Kitex/Thrift server on the configured address, registers the
