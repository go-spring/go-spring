// Package kitexthriftsvr wires Kitex/Thrift handlers into a single controller
// and registers it.
...
func init() {
	...
	// bean (see kitexthriftsvr.go) gates on this interface bean, mirroring how the
...
// GS_PROJECT_NAMEController composes the Kitex/Thrift order and user controllers
// into a single controller that satisfies the generated svc.GS_PROJECT_NAMEService
...
type GS_PROJECT_NAMEController struct {
	orderCtrl.KitexThriftOrderController
	userCtrl.KitexThriftUserController
}
