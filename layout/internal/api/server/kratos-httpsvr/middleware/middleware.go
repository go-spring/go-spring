// Package middleware provides kratos-HTTP middleware for the kratos-HTTP server.
package middleware

import "context"

// UnaryInterceptor is the prototype for kratos-HTTP unary interceptors. kratos
// unifies http and grpc middleware behind the same handler signature, so the
// http middleware mirrors the grpc one.
...
func Recovery(config RecoveryConfig) UnaryInterceptor {
	return func(next UnaryHandler) UnaryHandler {
		return func(ctx context.Context, req any) (resp any, err error) {
			// TODO: defer/recover and map the panic to a kratos error.
			return next(ctx, req)
		}
	}
}
