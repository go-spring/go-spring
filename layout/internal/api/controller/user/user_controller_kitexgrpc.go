...
// file suffix (user_controller_<proto>.go). This file is the Kitex/gRPC adapter;
// see user_controller.go for the HTTP baseline. All adapters delegate to the
// same user application service, so every protocol converges on one core.
package user
...
// KitexGrpcUserController adapts Kitex-generated (gRPC/protobuf) user calls to
// the user application service.
type KitexGrpcUserController struct {
...
func (c *KitexGrpcUserController) RegisterUser(ctx context.Context, req *svc.RegisterUserReq) (*svc.UserResp, error) {
	resp := &svc.UserResp{}
	u, err := c.UserService.RegisterUser(converter.FromKitexGrpcRegisterUserReq(req))
	if err != nil {
		resp.Code = 1
		resp.Message = err.Error()
		return resp, nil
	}
	resp.Data = converter.ToKitexGrpcUser(u, req.Email)
	return resp, nil
}
...
func (c *KitexGrpcUserController) UpgradeUser(ctx context.Context, req *svc.UpgradeUserReq) (*svc.UserResp, error) {
