...
// by file suffix (user_controller_<proto>.go). This file is the kratos-HTTP
...
// KratosHttpUserController adapts kratos-HTTP user rpcs to the user
// application service.
type KratosHttpUserController struct {
...
func (c *KratosHttpUserController) RegisterUser(ctx context.Context, req *pb.RegisterUserReq) (*pb.UserResp, error) {
	u, err := c.UserService.RegisterUser(converter.FromKratosHttpRegisterUserReq(req))
	if err != nil {
		return &pb.UserResp{Code: 1, Message: err.Error()}, nil
	}
	return &pb.UserResp{Code: 0, Message: "ok", Data: converter.ToKratosHttpUser(u, req.GetEmail())}, nil
}
...
func (c *KratosHttpUserController) UpgradeUser(ctx context.Context, req *pb.UpgradeUserReq) (*pb.UserResp, error) {
