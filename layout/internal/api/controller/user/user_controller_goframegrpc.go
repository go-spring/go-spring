...
// by file suffix (user_controller_<proto>.go). This file is the goframe gRPC
...
// GoframeGrpcUserController adapts goframe gRPC user rpcs to the user
// application service.
type GoframeGrpcUserController struct {
...
func (c *GoframeGrpcUserController) RegisterUser(ctx context.Context, req *pb.RegisterUserReq) (*pb.UserResp, error) {
	u, err := c.UserService.RegisterUser(converter.FromGoframeGrpcRegisterUserReq(req))
	if err != nil {
		return &pb.UserResp{Code: 1, Message: err.Error()}, nil
	}
	return &pb.UserResp{Code: 0, Message: "ok", Data: converter.ToGoframeGrpcUser(u, req.GetEmail())}, nil
}
...
func (c *GoframeGrpcUserController) UpgradeUser(ctx context.Context, req *pb.UpgradeUserReq) (*pb.UserResp, error) {
