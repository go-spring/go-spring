...
// by file suffix (user_controller_<proto>.go). This file is the goframe HTTP
...
// GoframeHttpUserController adapts goframe HTTP user rpcs to the user
// application service.
type GoframeHttpUserController struct {
...
func (c *GoframeHttpUserController) RegisterUser(ctx context.Context, req *pb.RegisterUserReq) (*pb.UserResp, error) {
	u, err := c.UserService.RegisterUser(converter.FromGoframeHttpRegisterUserReq(req))
	if err != nil {
		return &pb.UserResp{Code: 1, Message: err.Error()}, nil
	}
	return &pb.UserResp{Code: 0, Message: "ok", Data: converter.ToGoframeHttpUser(u, req.GetEmail())}, nil
}
...
func (c *GoframeHttpUserController) UpgradeUser(ctx context.Context, req *pb.UpgradeUserReq) (*pb.UserResp, error) {
