...
// FromKitexThriftCreateOrderReq converts a Kitex/Thrift CreateOrderReq to an
// application DTO. user_id is transported as a string over the wire and parsed
// into int64 here.
func FromKitexThriftCreateOrderReq(req *svc.CreateOrderReq) *dto.CreateOrderReq {
...
// ToKitexThriftOrder converts an order DTO to a Kitex/Thrift Order model.
func ToKitexThriftOrder(order *dto.OrderDTO) *svc.Order {
	return &svc.Order{
		ID:     order.ID,
