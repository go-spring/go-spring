...
2. **指标暴露**: gin 指标中间件记录的 `http.server.request_count`、
   `http.server.request_duration`、`http.server.active_requests`，
   通过 actuator 的 `/metrics` 端点（`:9370`）以 Prometheus 格式暴露。
...
# 应看到 http_server_request_count_total、http_server_request_duration_seconds_bucket
...
```text
 [测试进程]
  ...
  |       |-- 从请求头提取 traceparent
  |       |-- 创建 "HTTP GET" Span（abc123 的子 Span）
  |       |-- 设置 http.method, http.target, http.scheme, net.host.name 等属性
  |       |-- [handler 产生携带 trace_id=abc123 的日志，返回 traceparent+traceID]
  |       |-- 结束 Span，记录 http.status_code=200
  |
  |-- /metrics 抓取确认 http_server_request_count_total 已递增
