...
// Package main is the observability example for starter-gin.
//
...
// OTLP/gRPC to an OpenTelemetry Collector (docker-compose), which fans
// them out to Jaeger.
//
...
func init() {
	gs.Provide(func() StarterGin.RouterRegister {
		return func(e *gin.Engine) {
			e.GET("/echo/:name", func(ctx *gin.Context) {
				name := ctx.Param("name")
...
func runTest() {
	...
	for i := 0; i < 20; i++ {
		resp, err := http.Get("http://127.0.0.1:8001/echo/world")
		if err != nil {
	...
	traces, err := httpGet("http://127.0.0.1:16686/api/traces?service=gin-otel-example&limit=1")
	if err != nil {
		fmt.Fprintln(os.Stderr, "Jaeger API request failed:", err)
		os.Exit(1)
	}
	if !strings.Contains(traces, `"data":[`) {
		fmt.Fprintln(os.Stderr, "no traces found in Jaeger for service 'gin-otel-example'")
		os.Exit(1)
	}
	fmt.Println("OK: traces found in Jaeger for service 'gin-otel-example'")
