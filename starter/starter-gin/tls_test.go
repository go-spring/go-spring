...
// TestGinServer_MTLS verifies the server-side TLS wiring: ca-file enables
...
func TestGinServer_MTLS(t *testing.T) {
	dir := t.TempDir()
	...
	cfg := Config{
		...
		Middleware: MiddlewareConfig{Enabled: false},
	}
	...
	noCert := &http.Client{Transport: base, Timeout: 5 * time.Second}
...
// freeAddr grabs an ephemeral port the server can bind.
