...
// KV is a small demo model. We keep the same column names (`kkey` / `vvalue`)
// used across the other gorm starter examples so the code stays uniform.
type KV struct {
...
func main() {
	...
	http.HandleFunc("/sqlite_version", func(w http.ResponseWriter, r *http.Request) {
	...
	// Example usage:
	//
	// ~ curl http://127.0.0.1:9090/sqlite_version
	// 3.45.1%
}
