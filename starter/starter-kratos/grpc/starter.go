...
// Package StarterKratosGrpc integrates kratos' gRPC transport server into the
// Go-Spring server lifecycle. Import it for the side effect and provide a
// ServiceRegister bean to bind services:
//
//	import _ "go-spring.org/starter-kratos/grpc"
//
// Configuration is bound from the ${spring.kratos.grpc.server} prefix. Leave
...
// registration behind this function type keeps GrpcServer service-agnostic: it
// drives the lifecycle while each service supplies its own register bean,
// typically wrapping the generated xxx.RegisterXxxServer.
type ServiceRegister func(gs *kgrpc.Server) error

// Config binds kratos gRPC server + etcd registry configuration from
...
// GrpcServer adapts a kratos.App wrapping a single gRPC transport server to the
...
func (s *GrpcServer) Run(ctx context.Context, sig gs.ReadySignal) error {
	...
	opts := []kgrpc.ServerOption{kgrpc.Middleware(mw...)}
	if s.cfg.Network != "" {
		opts = append(opts, kgrpc.Network(s.cfg.Network))
	}
	if s.cfg.Addr != "" {
		opts = append(opts, kgrpc.Address(s.cfg.Addr))
	}
	if s.cfg.Timeout != 0 {
		opts = append(opts, kgrpc.Timeout(s.cfg.Timeout))
	}
	grpcSrv := kgrpc.NewServer(opts...)

	if err := s.reg(grpcSrv); err != nil {
		return errutil.Explain(err, "failed to register kratos grpc service")
	}
