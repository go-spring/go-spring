...
- 向 `cloud/data/cache` 注册名为 `bigcache` 的 driver，让使用
  `cache.Cache` 抽象的调用方（包括 `cloud/data/cache` 的 MultiLevel）能按名
...
- **`AsCache` 适配器接进 driver 注册表。** starter 对 `cloud/data/cache` 的
...
- **值类型是 `[]byte`。** 业务类型由调用方编解码；`cloud/data/cache`
...
- **自动 loader / refresh 缓存——否决。** loader 语义属于缓存抽象层
  （`cloud/data/cache`），不属于后端 starter。starter 贡献 store，抽象层贡献
  loader/refresh 策略。
