...
The call site — a generated client — rides the installed transport automatically; nothing to
inject:
...
```
custom driver wrap (outermost: embed DefaultDriver and wrap the assembled transport)
  ...
  → otelhttp.NewTransport(http.DefaultTransport | TLS-configured clone when tls.enabled)
  → the wire
```
...
- **executor stack order `observe(fault(rawExec))`** (httpx.go, the default wrap): fault injects *inside*
...
9. ~~No TLS keys at all~~ — FIXED 2026-08-28: `tls.*` block (enabled/cert-file/key-file/ca-file/
   ...
   route now has a certificate surface.
