...
// Greet returns a greeting for the request name.
...
func runTest() {
	...
	for i := 0; i < 20; i++ {
		_, err := proxy.Greet(context.Background(), &greet.GreetRequest{Name: "world"})
		if err != nil {
			fmt.Fprintln(os.Stderr, "Error calling Greet:", err)
			os.Exit(1)
		}
	}
	fmt.Println("Sent 20 Greet RPCs")

	// Wait for collector to flush.
	time.Sleep(3 * time.Second)

	// Verify traces appear in Jaeger.
	traces, err := httpGet("http://127.0.0.1:16686/api/traces?service=trpc-otel-example&limit=1")
	if err != nil {
		fmt.Fprintln(os.Stderr, "Jaeger API request failed:", err)
		os.Exit(1)
	}
	if !strings.Contains(traces, `"data":[`) {
		fmt.Fprintln(os.Stderr, "no traces found in Jaeger for service 'trpc-otel-example'")
		os.Exit(1)
	}
	fmt.Println("OK: traces found in Jaeger for service 'trpc-otel-example'")
