...
示例会访问 pprof 独立 HTTP 服务器（示例显式绑定 `127.0.0.1:9981`）上的三个代表性端点：
...
pprof 端点会暴露敏感的运行时内部信息（goroutine 栈、堆、CPU profile），因此绝不能在
无鉴权情况下被外部访问。默认地址为运维便利绑定到所有接口；当以此地址运行却未配置任何
鉴权时，starter 会在启动时打印告警——请设置 token 或用户名/密码，或将绑定限制为 loopback。
