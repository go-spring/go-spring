...
// Command example is a flagship for starter-memcached that composes the cloud-
// native capability set around a cache client:
//
//   - DISCOVERY: the memcached address is resolved through a registered discovery
...
func runTest(s *Service, c *Config) {
	...
	fmt.Println("health: actuator probes UP (memcached health.Indicator aggregated)")
	...
	fmt.Println("discovery: memcached resolved via backend -> Set/Get round-trip OK")
