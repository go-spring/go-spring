...
// Package main is the observability example for starter-memcached.
//
// It runs Memcached SET/GET/INCR/DELETE/Ping operations through an
// otel-instrumented client, verifies client spans reach Jaeger, then
...
// inject that type rather than the raw *memcache.Client.
type Service struct {
...
func runTest(s *Service) {
	...
	traces, err := httpGet("http://127.0.0.1:16686/api/traces?service=memcached-otel-example&limit=1")
	if err != nil {
		fmt.Fprintln(os.Stderr, "Jaeger API request failed:", err)
		os.Exit(1)
	}
	if !strings.Contains(traces, `"data":[`) {
		fmt.Fprintln(os.Stderr, "no traces found in Jaeger for service 'memcached-otel-example'")
		os.Exit(1)
	}
	fmt.Println("OK: traces found in Jaeger for service 'memcached-otel-example'")
