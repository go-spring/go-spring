...
`starter-websocket-coder` 基于 [coder/websocket](https://github.com/coder/websocket)
向 Go-Spring 服务提供一个配置好的 `*websocket.AcceptOptions`。它**不拥有** HTTP 服务，
也不占用端口：WebSocket 连接本质上只是一次 HTTP `Upgrade`，因此你应把 WebSocket
路由挂载到应用已经运行的任意 HTTP 服务上（net/http、gin、echo、hertz …）。

它是 `starter-websocket`（基于 gorilla/websocket）的 coder/websocket 兄弟版本。
与 gorilla 不同，coder/websocket 没有 `Upgrader` 对象，服务端升级是自由函数
`websocket.Accept(w, r, *AcceptOptions)`，因此本 starter 直接把
`*websocket.AcceptOptions` 作为可注入的 Bean 提供出来。
...
### 1. 引入 `starter-websocket-coder` 包

参见 [example.go](example/example.go) 文件。使用空导入即可——只需其 `init()` 注册
`*websocket.AcceptOptions` 提供者：
...
若需更多定制（如 `Subprotocols`），可自行提供 `*websocket.AcceptOptions` Bean，
`OnMissingBean` 会让你的实现优先生效。

### 3. 将 WebSocket 路由挂到 HTTP 服务

在注册 HTTP 路由处注入 `*websocket.AcceptOptions` 并调用 `websocket.Accept`。示例通过
提供自定义的 `*gs.HttpServeMux`，把路由挂到 gs 内置 HTTP 服务上：
...
* **文本回显 (`/echo`)** —— 完成 WebSocket 升级后，通过 `conn.Read` /
  `conn.Write` 原样回写每一帧文本消息。
* **JSON 回显 (`/json`)** —— 使用 `wsjson.Read` / `wsjson.Write` 接收
  `{"name": "..."}` 并回复 `{"message": "Hi, ..."}`。
