...
// once under the default backend name. The MongoDB instance configured with
// `service-name` (see conf/app.properties) then dials the address this backend
// hands out, with no per-component wiring.
//
...
func init() {
	discovery.Register("default", staticDiscovery{addr: "127.0.0.1:27017"})
}
