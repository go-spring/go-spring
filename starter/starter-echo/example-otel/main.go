...
// Package main is the observability example for starter-echo.
//
...
func init() {
	gs.Provide(func() StarterEcho.RouterRegister {
		return func(e *echo.Echo) {
			e.GET("/echo/:name", func(ctx echo.Context) error {
				name := ctx.Param("name")
				log.Infof(ctx.Request().Context(), log.TagAppDef, "handling /echo/%s", name)
				return ctx.JSON(http.StatusOK, map[string]string{"message": "Hello, " + name})
			})
		}
	})
}
...
func runTest() {
	...
	for i := 0; i < 20; i++ {
		resp, err := http.Get("http://127.0.0.1:8002/echo/world")
		if err != nil {
	...
	traces, err := httpGet("http://127.0.0.1:16686/api/traces?service=echo-otel-example&limit=1")
	if err != nil {
		fmt.Fprintln(os.Stderr, "Jaeger API request failed:", err)
		os.Exit(1)
	}
	if !strings.Contains(traces, `"data":[`) {
		fmt.Fprintln(os.Stderr, "no traces found in Jaeger for service 'echo-otel-example'")
		os.Exit(1)
	}
	fmt.Println("OK: traces found in Jaeger for service 'echo-otel-example'")
