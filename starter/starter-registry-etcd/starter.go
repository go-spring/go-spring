...
// UpdateWeight re-advertises this instance with a new weight on its existing
// lease: the entry never leaves discovery and watchers (loadbalance pools
// included) simply observe the new value on their next snapshot. It is an
// optional runtime API — call it after Run has registered the instance.
func (s *Server) UpdateWeight(ctx context.Context, weight int) error {
	if s.registrar == nil || s.reg.Addr == "" {
		return errutil.Explain(nil, "registry-etcd: instance not registered yet")
	}
