...
// Command example demonstrates wiring starter-registry-k8s into a Go-Spring
// application. Blank-importing the starter and declaring one
// "${spring.registry.k8s.<name>}" block provides a discovery backend bean named
// "k8s.<name>"; any client (Redis/GORM/...) then resolves a Kubernetes Service
// by setting its `discovery:` field to that bean name.
//
...
import (
	...
	// Blank-import provides the Kubernetes discovery backend bean(s) declared
	// under spring.registry.k8s.
	_ "go-spring.org/starter-registry-k8s"
)

// backendName matches the block name in conf/app.properties
// (spring.registry.k8s.<block> provides the bean "k8s.<block>"); targetService
// is the Kubernetes Service name to resolve through it.
const (
	backendName   = "k8s.main"
	targetService = "demo"
)
...
type resolveRunner struct {
	// backend is the discovery bean named after the spring.registry.k8s.<block>
	// entry in conf/app.properties ("k8s.<block>").
	backend discovery.Discovery `autowire:"k8s.main"`
}
