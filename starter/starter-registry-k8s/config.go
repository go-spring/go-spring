...
// Package StarterRegistryK8s integrates Kubernetes-native service discovery
...
// "${spring.registry.k8s.<name>}". The block key <name> becomes the bean name
// "k8s.<name>" that a client starter references via its `discovery:` field
// (e.g. redis `discovery: k8s.main`). The service name passed to Resolve/Watch
// is the Kubernetes Service name; this Config supplies the surrounding context
// (namespace, port selection, auth) that turns it into live endpoints.
//
// This backend contributes a discovery.Discovery only — no registrar. The
// family-wide registration keys (${spring.registry.service-name} / .addr)
// belong to the registry core and are deliberately not bound here: in a cluster
...
func (c Config) validate() error {
	...
	case ModeDNS:
		if c.PortName == "" && c.Port <= 0 {
			return errutil.Explain(nil, "registry-k8s: dns mode requires port-name (SRV) or port (A record)")
		}
	...
	default:
		return errutil.Explain(nil, "registry-k8s: invalid mode %q (want %q or %q)", c.Mode, ModeDNS, ModeEndpointSlice)
	}
	return nil
}
