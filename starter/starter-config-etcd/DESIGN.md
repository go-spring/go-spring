...
- Registers an `etcd` provider name via `conf.RegisterProvider` in `init()` and
  nothing else at the package top level — no exported bean for users, no server
  (the internal controller bean is plumbing, registered by the same init).
- Parses the provider source `etcd:<host>:<port>/<key>?<query>`, reads the key,
  parses the payload (`properties`/`yaml`/`toml`/`json` via the shared conf
  reader registry), and flattens it into a `map[string]string` for the framework
  to merge.
