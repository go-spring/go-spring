...
// Package main is the observability example for starter-goframe/http.
//
// It starts a GoFrame HTTP server with tracing and metrics middleware
...
func runTest() {
	...
	fmt.Println("Sent 20 requests to /hello")

	// Wait for collector to flush.
	time.Sleep(3 * time.Second)

	// Verify traces appear in Jaeger.
	traces, err := httpGet("http://127.0.0.1:16686/api/traces?service=goframe-http-otel-example&limit=1")
	if err != nil {
		fmt.Fprintln(os.Stderr, "Jaeger API request failed:", err)
		os.Exit(1)
	}
	if !strings.Contains(traces, `"data":[`) {
		fmt.Fprintln(os.Stderr, "no traces found in Jaeger for service 'goframe-http-otel-example'")
		os.Exit(1)
	}
	fmt.Println("OK: traces found in Jaeger for service 'goframe-http-otel-example'")
