...
[starter/DESIGN.md](../DESIGN.md) §2.3),为 `stdlib/session`(Spring Session
的 Go-Spring 等价物)提供基于 Redis 的 `session.SessionStore`。不开端口;
贡献一个 `SessionStore` bean,由 `stdlib/session.Manager` 用于跨副本加载
与保存会话。
...
- **不在范围内:**HTTP `Manager` 中间件与 cookie 处理(那是
  `stdlib/session` 的事);redis 客户端本身(那是 `starter-go-redis`)。

## 2. 关键抽象与缝隙

`stdlib/session` 分三段拆分,让 starter 无需触碰 HTTP 就能接入:
...
- **走 bean,而非 driver 注册表。**Session 需要应用已装配的**活** redis
  ...
  `stdlib/session` 的 driver 注册表只服务静态默认(`"memory"`);Redis 走
...
## 4. Manager 决策(上下文,住在 `stdlib/session`,此处镜像)
