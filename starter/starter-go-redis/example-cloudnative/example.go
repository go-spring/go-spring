...
// Command example is a flagship for starter-go-redis that composes the cloud-
// native capability set around a cache client:
//
...
func init() {
	...
	// The redis client resolves "redis-cluster" through this "default" backend.
	gs.Provide(func() (discovery.Discovery, error) {
		return discovery.NewStaticDiscovery(discovery.Endpoint{Addr: "127.0.0.1:6379", Healthy: true}), nil
	}).Name("default")
}

// Service autowires the "cache" redis instance. Its address is resolved via
...
func runTest(s *Service, c *Config) {
	...
	fmt.Println("health: actuator probes UP (redis health.Indicator aggregated)")

	// --- 2. Discovery -----------------------------------------------------
	// The instance's address comes from the discovery backend, not config. A
	// successful round-trip proves resolve -> dial -> serve.
	if _, err := s.Redis.Set(ctx, "cn:key", "cn-value", 0).Result(); err != nil {
		fail("redis via discovery: %v", err)
	}
	if got, err := s.Redis.Get(ctx, "cn:key").Result(); err != nil || got != "cn-value" {
	...
	for range 15 {
		_, err := s.Redis.Set(ctx, "cn:rl", "x", 0).Result()
		switch {
		case err == nil:
			admitted++
