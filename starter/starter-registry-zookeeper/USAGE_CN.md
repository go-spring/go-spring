...
`registrar.go`、`center.go`、`discovery_zookeeper.go`、`config.go`、`registrar_test.go`）、`cloud/discovery` 接缝（`cloud/discovery/discovery.go`、
...
开关（`starter.go`）。同一 key 还会创建共享中心连接 bean（`center.go`），并在其上派生标签为
`discovery-name`（默认 `zookeeper`）的 discovery 后端 bean。本 starter **两侧都做**：把本实例以临时
znode 发布到 ZooKeeper，同时携带客户端发现后端（见 §1 消费侧说明）。不监听端口——导出 `gs.Server`
纯粹为了把注册接进应用生命周期。
...
**消费侧**。本 starter 自带 ZooKeeper 发现后端（`discovery_zookeeper.go`）：一个命名
`discovery.Discovery` bean，列举 `<base-path>/<service-name>` 的 children（每个 child 名即实例
id），`Get` 各 znode 数据，解码自描述的 `instanceValue` JSON，映射到
`discovery.Endpoint{Addr, Weight, Metadata}`，再用 `ChildrenW`/`GetW` watcher 保持快照新鲜
（watch 循环形态见 §2.3）。获得该 bean 有两种方式：

- **从中心派生（双角色）**：只配置 `spring.registry.zookeeper`，即会在共享中心连接上派生一个标签为
  `discovery-name`（默认 `zookeeper`）的后端 bean——无需任何 discovery 块。置
  `discovery-name=`（空）可禁用。
- **独立块（其他集群 / 纯消费端）**：在 `spring.discovery.zookeeper.<name>` 下每个集群一块；
  `servers` 留空继承中心连接，非空则自建（并探测）连接。
...
spring.redis.demo.discovery=zookeeper   # bean 标签：派生名或块名
```
...
gs.Run()
  ...
  └─ SIGTERM：PreStop → 最先 Deregister（早于 pre-stop 延迟、早于任何 server
       停止），发现侧在存量请求排空期间就不再分发本实例；Stop/Stop 幂等
...
自带的发现后端（`discovery_zookeeper.go`）其 watch 循环与 pool 消费
snapshot 的方式互为镜像：
...
已用 `grep -rhoE 'value:"[^"]+"' ... | sort -u` 核对——11 个 key（下表 10 个加 `discovery-name`）+ 结构体级的 `${spring.registry}`
...
| `spring.registry.zookeeper.discovery-name` | string | `zookeeper` | 在该标签下为同一集群派生 discovery 后端 bean（共享中心连接）；留空禁用（`center.go`） | 标签与其他 bean 名冲突时容器内报错，fail loud |
...
4. ~~不带消费侧~~ 已解决：`discovery_zookeeper.go` 自带发现后端（中心派生 + 独立块），
   consul 式缺口已闭合。
