...
客户端服务发现(`cloud/discovery`)的注册侧对应物 —— **同时**提供消费侧的 ZooKeeper
discovery 后端,一个 starter 同时服务于命名习语的两半。它相当于 Spring Cloud
`ServiceRegistry` + `DiscoveryClient`,以临时节点(ephemeral znode)为底座。
...
| `discovery-name` | `zookeeper` | 在该标签下为同一集群派生一个 discovery 后端 bean —— 一份配置同时服务两半(共享连接)。留空则禁用派生后端。 |
...
消费侧是一个 `cloud/discovery` 后端 bean,有两种获得方式:

**从中心派生(双角色应用)。** 只设置 `spring.registry.zookeeper`,即会在同一共享连接上
派生一个标签为 `discovery-name`(默认 `zookeeper`)的后端 bean —— 一份集群配置,两半
齐备:
...
spring.registry.service-name=orders
spring.registry.addr=10.0.0.5:8080
# 客户端随后引用: <client>.discovery=zookeeper
```

**独立块(其他集群 / 纯消费端)。** 在 `spring.discovery.zookeeper.<name>` 下每个集群
一块配置,client starter 按名字引用:
...
| 键 | 默认值 | 说明 |
| --- | --- | --- |
| `servers` | (空) | 集群成员;留空即**继承** `spring.registry.zookeeper` 中心连接(共享会话)。 |
| `session-timeout` | `10s` | 约束独立块的初始连接与启动探测。 |
| `base-path` | `/services` | 必须与注册方应用的 base 路径一致。 |
| `username` / `password` | (空) | digest 认证凭据,开放集群留空。 |

`base-path` 必须与注册方一致,否则解析不到任何实例。健康度由 znode 存活推导:实例是一个
临时节点,注册方会话一死 ZooKeeper 就把它删除,因此列举到的每个节点都是存活实例 —— 无需
探测协议。注册实例上可选的 `scheme` metadata 键承载传输选择,与 etcd 适配器对齐。

## 工作原理

- 在 bean 构造阶段,starter 为 ${spring.registry.zookeeper} 构建共享中心连接(探测集群
  —— 一次 `Exists` 调用会阻塞到会话连上 —— 不可达的 ZooKeeper 会让启动失败),在其上
  构建 ZooKeeper registrar 并注入导出的 `gs.Server`。
