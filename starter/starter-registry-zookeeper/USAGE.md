...
`config.go`, `registrar_test.go`), the
...
set — that key is the on/off switch (`starter.go`). The same key creates the shared center
connection bean (`center.go`), and the center derives a discovery backend bean labeled
`discovery-name` (default `zookeeper`) on it. This starter serves **both sides**: it advertises this
instance to ZooKeeper as an ephemeral znode AND ships the client-side discovery backend (see §1
consumer note). It opens no port — it exports a `gs.Server` purely to plug
...
**Consumer side.** This starter ships the ZooKeeper discovery backend (`discovery_zookeeper.go`):
a named `discovery.Discovery` bean that lists the children of `<base-path>/<service-name>` (each
child name is the instance id), `Get`s each znode's data, decodes the self-describing
`instanceValue` JSON, maps to `discovery.Endpoint{Addr, Weight, Metadata}`, and keeps the snapshot
fresh with `ChildrenW`/`GetW` watchers (see §2.3 for the watch loop shape). Two ways to get the
bean:

- **Derived from the center (dual-role)**: configuring `spring.registry.zookeeper` alone derives a
  backend bean labeled `discovery-name` (default `zookeeper`) on the shared center connection — no
  discovery block needed. Set `discovery-name=` (empty) to disable it.
- **Standalone block (other ensembles / pure consumers)**: one block per ensemble under
  `spring.discovery.zookeeper.<name>`; `servers` empty inherits the center connection, non-empty
  builds (and probes) its own.

```properties
# standalone block example
spring.discovery.zookeeper.prod.servers=10.0.0.9:2181
spring.discovery.zookeeper.prod.base-path=/services

spring.redis.demo.service-name=orders
spring.redis.demo.discovery=zookeeper   # the bean label: derived name or block name
```
...
gs.Run()
  ...
  └─ on SIGTERM: PreStop → Deregister FIRST (before the pre-stop delay and any
       ...
       requests drain; Stop/Stop deregister again as idempotent
...
Verified against `grep -rhoE 'value:"[^"]+"' ... | sort -u` — 11 keys (10 below plus
`discovery-name`) + the struct-level
...
| `spring.registry.zookeeper.discovery-name` | string | `zookeeper` | derives a discovery backend bean for the same ensemble under that label (shared center connection); empty disables it (`center.go`) | a label colliding with another bean name fails loudly in the container |
...
4. ~~No shipped consumer side~~ RESOLVED: `discovery_zookeeper.go` ships the backend (center-derived
   + standalone blocks), closing the consul-style gap.
