...
// Package main is the observability example for starter-gorm-mysql.
//
// It runs GORM CRUD + transaction operations through an opentelemetry-instrumented
// client, verifies client spans reach Jaeger, then self-exits. Traces are exported
...
func runTest(s *Service) {
	...
	// registered discovery backend (service-name=mysql-cluster), not from
	...
	traces, err := httpGet("http://127.0.0.1:16686/api/traces?service=gorm-mysql-otel-example&limit=1")
	if err != nil {
	...
	fmt.Println("OK: traces found in Jaeger for service 'gorm-mysql-otel-example'")
