...
//
// It needs a reachable Consul agent at the configured address; check.sh starts
// one in Docker.
package main
...
import (
	...
	// Blank-import registers the Consul registrar, the register-on-ready
	// server, and the discovery backend derived from the center config.
...
func (v *VerifyRunner) verify(ctx context.Context) {
	...
	if err != nil {
		...
		log.Warnf(ctx, log.TagAppDef, "service orders has no healthy instances yet")
	}
	for _, e := range eps {
