...
type ConsulConfig struct {
	...
	// DiscoveryName derives a discovery backend bean for this same agent under
	// that label — the "one config block serves both halves" default: a
	// dual-role application configures the agent once and its clients cite
	// discovery=<discovery-name>. Empty disables the derived backend (a
	// write-only application, or one whose consumers use explicit
	// ${spring.discovery.consul.<name>} blocks).
	DiscoveryName string `value:"${discovery-name:=consul}"`
}
