...
// "vault" config provider (see provider.go) that can be consumed via
// spring.app.imports, together with the bridge that wires secret changes into
...
// exposes its fields as application properties. It pairs naturally with the
// property-level decryption seam in spring/conf/decrypt — a secret value can
// still be an ENC(...) payload — but the two are independent. A Vault Agent /
// CSI-mounted secret file is read with starter-config-file instead; this
// starter talks to the Vault API directly.
...
func init() {
	...
	// It links the "vault" remote config provider's polling watcher to the
	...
	gs.Provide(newConfigRefreshBridge).
		Name("vaultConfigRefreshBridge").
		Export(gs.As[gs.Rooter]())
}

// configRefreshBridge connects remote Vault secret changes to the
// application-wide property refresh mechanism.
type configRefreshBridge struct{}

// newConfigRefreshBridge installs the refresh hook used by the "vault" config
// provider. It injects the framework's PropertiesRefresher so that a secret
