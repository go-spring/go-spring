...
This works through the zero-dependency `go-spring.org/cloud/actuator/endpoint` seam:
...
`starter-otel` does **not** install a trace↔log hook itself. The
`log.FieldsFromContext` seam in `go-spring.org/log` is a single process-wide
hook point — too narrow to cover the range of correlation strategies an app
might want — so the starter leaves it to the application to own. Installing the
canonical hook (stamp every record with the active span's `trace_id`/`span_id`)
is a few lines:
...
Call it once at startup; then a request's logs join its trace with no
per-call-site code:
...
code paths pay nothing. The [example](example) installs this hook itself.
...
once is enough to (1) correlate a request's logs to its trace via `trace_id`
(the example installs the `log.FieldsFromContext` hook itself) and (2) serve
otel's Prometheus `/metrics` on the actuator management port — with no
