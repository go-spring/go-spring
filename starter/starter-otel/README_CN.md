...
这通过零依赖的 `go-spring.org/cloud/actuator/endpoint` 缝隙实现：`starter-otel` 把抓取
...
`starter-otel` **不**自行安装 trace↔log 钩子。`go-spring.org/log` 的 `log.FieldsFromContext` 缝隙是一个进程级单点钩子——覆盖面太窄，难以满足应用可能想要的各种关联策略，因此 starter 把它交给应用自己掌握。安装标准钩子（在每条日志记录上盖当前 span 的 `trace_id`/`span_id`）只需几行：
...
启动时调用一次即可；此后一个请求的日志自动与其 trace 关联，无需任何逐调用点代码：
...
当 context 不携带有效 span 时该钩子为 no-op，因此未埋点的代码路径零开销。[示例](example)自行安装了该钩子。
...
将一个请求的日志关联到其 trace（示例自行安装 `log.FieldsFromContext` 钩子），（2）
在 actuator 管理端口上暴露 otel 的 Prometheus
