...
| `trace.propagator` | string | w3c | w3c = TraceContext+Baggage composite; none leaves the process default untouched — returns nil, does not clear (trace/provider.go:78-90). ⚠ Ignored entirely when trace pillar is off. | Typo → setup error `unknown propagator (want w3c|none)`. |
