...
`spring.discovery.nacos.<name>` has entries (a block with empty `server` inherits the
center connection). No `enabled` key anywhere; the registrar block itself derives a
discovery bean labeled `discovery-name` (default `nacos`).
...
gs.Run()
  ...
  └─ SIGTERM: PreStop → Deregister FIRST (before pre-stop delay, before any server
       ...
       Stop/Stop deregister again as fallback; deregister is idempotent
```
...
| Key | Type | Default | Behavior | Misconfiguration consequence |
|-----|------|---------|----------|------------------------------|
| `server` | string | "" | Empty INHERITS the `${spring.registry.nacos}` center connection (shared naming client). Duplicate `<name>` → container duplicate-bean error. | Both empty → injection-time error "no ... center is configured". |
...
Backends are NAMED BEANS in the IoC container (bean name = label): a client
starter injects the backend its `discovery` key cites. A dual-role app usually
needs no block at all: `spring.registry.nacos.discovery-name` (default `nacos`)
derives a backend bean for the same server/namespace/group/cluster from the
center config, so read and write cannot diverge (gateway: `spring.gateway.discovery` /
per-route `upstream.discovery`).
