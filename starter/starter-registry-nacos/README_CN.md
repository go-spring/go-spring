...
客户端服务发现(`cloud/discovery`)的注册侧对应物,相当于 Spring Cloud Alibaba
...
注销。别处的客户端通过 `discovery.Discovery` 后端按服务名解析到它——默认即从本配置块
派生的后端 bean(`discovery-name`,默认 `nacos`;client 引用 `discovery=nacos`),
也可用独立的 `${spring.discovery.nacos.<name>}` 块指向其它 server/纯消费方
(块内 `server` 留空则继承本块连接)。
...
| `discovery-name` | `nacos` | 为同一 server(共享 naming client、同 namespace/group/cluster)派生一个该标签的发现后端 bean——一份配置块服务两半。留空则不派生。 |
