...
func NewServer(c NacosConfig, ec *nacosCenter) (*Server, error) {
	log.Debugf(context.Background(), starterTag, "creating nacos registrar server=%s group=%s", c.Server, c.Group)
	if ec == nil {
		// Direct construction (e.g. tests): build a standalone probed client.
		var err error
		ec, err = newNacosCenter(c)
		if err != nil {
			return nil, err
		}
	}
	reg, err := newNacosRegistrar(c, ec.client)
	if err != nil {
