...
func TestNacosDiscovery_Resolve(t *testing.T) {
	...
	// The full metadata rides along, not just the scheme key: zone-aware
	// selection reads zone/version off Endpoint.Metadata.
	if eps[1].Metadata["scheme"] != "tls" {
		t.Fatalf("endpoint metadata dropped: %+v", eps[1])
	}
