...
`discovery.Discovery` backend for the same registry — by default the backend
bean derived from this very block (`discovery-name`, default `nacos`; clients
cite `discovery=nacos`), or a standalone
`${spring.discovery.nacos.<name>}` block for another server / pure consumers
(a block with empty `server` inherits this block's connection).
...
| `discovery-name` | `nacos` | Derives a discovery backend bean for this same server (shared naming client, same namespace/group/cluster) under that label — one config block serves both halves. Empty disables the derived backend. |
