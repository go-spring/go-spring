...
| `spring.transaction.at.tracing` | bool | `true` | Appends `at.WithObserver(transactionobserve.AtObserver{})` at coordinator construction (starter.go:93-94). Span per branch phase: `at.commit <branch>` / `at.rollback <branch>`, attributes `at.xid` / `at.branch` / `at.phase` (cloud/observe/transaction/observer.go:129-142). | `false` (or forgetting starter-otel with `true`): silent — no spans, no warning. |
