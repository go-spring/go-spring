...
（`starter.go`、`config.go`、`client.go`、`command.go`、`driver.go`、`binder.go`）及可运行的
...
一个经 binder（broker 中立信封）收发的服务，附裸 SDK 逃生口、actuator 与 OTel。文件树：
...
**service.go** —— 业务消息走 binder，一次关键发送走 guarded 裸路径：
...
spring.rocketmq.a.observability.level=brief   # GuardedSend 调用的访问日志
...
```bash
grep _app_rocketmq_access app.log | tail -2   # binder publish/consume 记录
...
- producer 创建即启动（NewProducer 内 `p.Start()` [client.go:116]）；consumer 返回
  **未启动**——需自行先 Subscribe 后 Start，或交给 binder [client.go:130-135]。
...
- executor 只包裹 `GuardedSend` 的同步 `SendSync`。**未守护**：binder 的 `Publish`
  （直接调 `p.p.SendSync` [binder.go:98]）、裸 `SendSync`，以及有意不碰的
...
1. 为 publisher 的固定 topic 构建 `primitive.Message`；`Key`（单个字符串）经 `WithKeys`
   成为消息 key [binder.go:87-88]；每个 Header 写入 user property [binder.go:89-91]。
2. 若 ctx 携带压测标记，写入 `x-loadtest=1` user property [binder.go:92-96; traffic.go:60]。
3. `startProduce` 打开 observe kit 的生产观测（"publish"）并把 W3C traceparent 注入
   user properties [command.go:152-156]——无 starter-otel 时为 no-op。
4. 裸 `SendSync`（绕过 resilience executor——见 §2.2）。
5. `sp.End(err)` 记录时长直方图、平衡 in-flight 计数、结束 span 并输出
   `_app_rocketmq_access` 日志记录 [observer.go:268-306]。

### 2.4 一次 consume 逐层走读（binder 路径）
...
4. `fromMessageExt` 构建信封：`Key` 取 KEYS property、`Payload` = body、
   `Headers` = **全部** user properties、`Timestamp` 取 StoreTimestamp [binder.go:154-161]。
5. handler 出错 → Error 日志 + `ConsumeRetryLater`（broker 按 RocketMQ 重试语义重投）；
   成功 → `ConsumeSuccess` [binder.go:136-141]。

已知映射丢失（双向）——均在 binder.go 核对：

| 字段 | Publish（信封 → RocketMQ） | Consume（RocketMQ → 信封） |
|---|---|---|
| Key | 单字符串 → 单个消息 key [binder.go:87] | KEYS property 读回为一个字符串；多 key 生产方得到拼接值而非原列表 [binder.go:156] |
| Headers | 条目原样写入 user properties [binder.go:89] | 返回**全部** properties，含 SDK 内部项（`traceparent`、KEYS、`x-loadtest`）——会漏进 `msg.Headers` [binder.go:158] |
| Tags | **无映射**——`messaging.Message` 无 tag 概念；binder 发的是无 tag 消息 | **无映射**——订阅硬编码选择器 `TAG *` [binder.go:122-124]；按 tag 过滤消费需走裸客户端 |
| Timestamp | 不发送 | StoreTimestamp（broker 存储时间），非 BornTimestamp [binder.go:159] |
...
绝对属性的 Pool 规则）。starter 内 9 个 value tag 加共享 `observability` 组的 3 个子键
——已与 grep 结果比对，两边均无多余项。
...
| `observability.level` | string | `brief` | **guarded** 调用（resilience 观察器）的访问日志；`off`/`brief`/`detailed` [config.go:68; observe/config.go:50]。binder 路径无论该值如何都固定 `brief`（包级默认观察器 [command.go:146-148]）。 | `off` 只静默 GuardedSend 访问日志，静默不了 binder 的。 |
| `observability.maxArgBytes` | int | `512` | detailed 模式下捕获参数的截断上限（observe/config.go:56）。 | 过小 → 参数被截断。 |
| `observability.skipOps` | list | — | 对 guarded 观察器按操作名同时压制 span+metric+log。 | — |
...
- binder 观察器：直方图 `messaging.client.operation.duration`（单位 s）与 gauge
  `messaging.client.active_requests`，属性 `messaging.system=rocketmq`、
  `messaging.operation=publish|consume` [observer.go:184-196]。访问日志 tag
  `_app_rocketmq_access`：字段 `messaging.operation`、`status`、`duration_ms`
  （detailed 模式追加截断后的参数）；成功 Info、出错 Warn [observer.go:305-320]。
...
- 压测标记演练：在带流量层标记的 ctx 下 publish；consumer handler 的
  `traffic.IsLoadTest(ctx)` 经 `x-loadtest` property 变为真 [binder.go:92-96, 131-133]。
...
| binder 发布毫无 resilience 效果 | Publish 按设计绕过 executor | 需要保护的发送用 `GuardedSend`（§2.2）。 |
...
| consumer 不停重投 | handler 持续返回 error → ConsumeRetryLater | 修 handler；binder 无 DLQ 接线——需要时用裸客户端配合 RocketMQ 重试/DLQ 语义。 |
...
| 指标 | 数值 |
|------|------|
| 配置 key 总数 | 11（Config 9 个 + observability 3 个子键，1 个共享组） |
| 其中必填 | 1（`name-servers`） |
...
- binder Subscribe 硬编码 `TAG *` 选择器；不支持 tag 子表达式，且
  `messaging.Message` 根本无法携带 tag（§2.4 表）（承接，未修）。
- binder Timestamp 用 StoreTimestamp 而非出生时间（承接，未修）。
- binder 发布不经过守护而裸路径可以——两条生产路径在保护/可观测上不对称
  （承接，未修）。
