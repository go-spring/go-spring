...
[starter/DESIGN.md](../../DESIGN.md) §2.3),基于零依赖抽象 `cloud/experimental/security`
...
- **不在范围内:**签发 token(那是 `starter-oauth2-server` 的事);授权
  *策略*(那是 `cloud/experimental/security` 的 aspect / middleware);用户存储 / 登录 UI。

## 2. 关键抽象与缝隙

- **`cloud/experimental/security`——零依赖抽象。**本 starter 是其中一种具体实现。
