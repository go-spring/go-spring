...
func (c *jwksCache) reload() error {
	...
	if err != nil {
		return errutil.Explain(err, "oauth2-resource-server: build JWKS request")
	}
	resp, err := c.client.Do(req)
	if err != nil {
		return errutil.Explain(err, "oauth2-resource-server: fetch JWKS %s", c.url)
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&doc); err != nil {
		return errutil.Explain(err, "oauth2-resource-server: decode JWKS %s", c.url)
	}
...
func (k jwk) rsaKey() (*rsa.PublicKey, error) {
	...
	if err != nil {
		return nil, errutil.Explain(err, "oauth2-resource-server: decode RSA modulus")
	}
	eBytes, err := base64.RawURLEncoding.DecodeString(k.E)
	if err != nil {
		return nil, errutil.Explain(err, "oauth2-resource-server: decode RSA exponent")
	}
...
func (k jwk) ecKey() (*ecdsa.PublicKey, error) {
	...
	if err != nil {
		return nil, errutil.Explain(err, "oauth2-resource-server: decode EC x")
	}
	yBytes, err := base64.RawURLEncoding.DecodeString(k.Y)
	if err != nil {
		return nil, errutil.Explain(err, "oauth2-resource-server: decode EC y")
	}
