...
// Package main is the observability example for starter-gorm-clickhouse.
//
// It runs ClickHouse queries through an otel-instrumented GORM client,
...
func runTest(s *Service) {
	...
	traces, err := httpGet("http://127.0.0.1:16686/api/traces?service=gorm-clickhouse-otel-example&limit=1")
	if err != nil {
		fmt.Fprintln(os.Stderr, "Jaeger API request failed:", err)
		os.Exit(1)
	}
	if !strings.Contains(traces, `"data":[`) {
		fmt.Fprintln(os.Stderr, "no traces found in Jaeger for service 'gorm-clickhouse-otel-example'")
		os.Exit(1)
	}
	fmt.Println("OK: traces found in Jaeger for service 'gorm-clickhouse-otel-example'")
