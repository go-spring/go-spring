...
调用侧——生成式或命令式——自动骑在已安装的 transport 上,无需注入:
...
// 命令式一次性调用(api.go):同一 transport、同一路由 key。
...
```
用户 TransportMiddleware(WrapTransport;最外层,可见完整内置栈)
  ...
  → otelhttp.NewTransport(http.DefaultTransport | tls.enabled 时为配置过 TLS 的克隆)   (或 SetBaseTransportFactory 的 base)
  → 网络
```

设计理由(均引自源码注释,已核实):

- **otelhttp 作 base**:trace 传播通过传入插桩的 Base transport 叠加,"keeping
  observability a starter concern"(httpx.go:37-38)。每次尝试——含重试——都被 trace 并
  注入 `traceparent`。
...
- **executor 栈序 `observe(fault(rawExec))`**(config.go:110-117):fault 注入在真实
...
### 2.4 扩展点(extension.go)

- `SetBaseTransportFactory(BaseTransportFactory)` ——替换默认的
  `otelhttp.NewTransport(http.DefaultTransport)` base;获得代理、连接池等 `tls.*` 之外的定制
  的唯一途径。返回 nil 可按 name 局部退出。须在容器装配前调用。
- `UseTransportMiddleware(TransportMiddleware)` ——包裹完整装配链;注册顺序 = 最外层
  优先。
- 命令式助手 `Get/Post/Put/Delete/Call[T]`、`WithQuery`、`WithScheme`(api.go)——同一
  `httpclt.DoRequest` seam,路由与保护和生成式客户端完全一致。
...
entry 才回退 `http:<addr>`(starter.go `httpResourceLabel`)。因此按 `http:<service-name>`
...
(starter.go `minRequestsFloor`)——resilience 自身的零值下限是 1,低流量下单次失败即熔断,
...
中心注销实例后,Resolver watch 会将其移出轮换(httpx.go:170-172)。
...
- trace:otelhttp base 每请求一个 client span(命名遵循
