...
// Package main is the observability example for starter-http-client.
//
// It exercises the four acceptance outcomes against real in-process backends
// (direct address, service discovery + load balancing, trace propagation,
// resilience), exports traces via OTLP/gRPC to Jaeger (docker-compose),
// verifies client spans reach Jaeger, then self-exits.
//
...
func runTest(s *Service) {
	...
	traces, err := httpGet("http://127.0.0.1:16686/api/traces?service=http-client-otel-example&limit=1")
	if err != nil {
		fmt.Fprintln(os.Stderr, "Jaeger API request failed:", err)
		os.Exit(1)
	}
	if !strings.Contains(traces, `"data":[`) {
		fmt.Fprintln(os.Stderr, "no traces found in Jaeger for service 'http-client-otel-example'")
		os.Exit(1)
	}
	fmt.Println("OK: traces found in Jaeger for service 'http-client-otel-example'")
...
// httpGet performs an HTTP GET and returns the response body as a string.
