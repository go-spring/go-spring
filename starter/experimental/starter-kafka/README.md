...
Register the driver as a bean from a `*kgo.Client` (select the named instance
with `gs.TagArg`):
...
transactions, the admin API and other Kafka features the driver does not model.
