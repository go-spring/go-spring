...
// `service-name` (see conf/app.properties) resolves its address through it.

func init() {
	gs.Provide(func() (discovery.Discovery, error) {
		return discovery.NewStaticDiscovery(discovery.Endpoint{Addr: "127.0.0.1:7687", Healthy: true}), nil
	}).Name("default")
}
