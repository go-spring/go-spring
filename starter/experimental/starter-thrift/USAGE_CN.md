...
gs.Run()
  ...
  │   ├─ newTransport()：监听（tls.enabled 时经 security.BuildClient() 建 TLS）← socket 在此打开
...
`grep -rhoE 'value:"[^"]+"' … | sort -u` 核对（10 个 tag；tls 子 key 来自 cloud/security）。
...
| `tls.cert-file` / `tls.key-file` | string | — | ⚠ `tls.enabled=true` 时必须成对提供（否则 `security.BuildClient()` 报错 → listen 失败）。 | 启动期 listen 报错 "thrift: build TLS"。 |
| `tls.ca-file` / `tls.server-name` / `tls.insecure-skip-verify` | — | — | **服务端死 key**：它们是 security 的客户端校验项；服务端路径只调 `Build()`，从不校验客户端证书（无 mTLS）。 | 期望 mTLS → 静默缺失。 |
...
| 期望 mTLS 但客户端没被校验 | 服务端走 `security.BuildClient()`；`ca-file` 等是死 key | 不支持；需要就作为设计问题提出。 |
...
5. example-otel conf 用了死的 `spring.thrift.server.interceptor.*` key——只因 observer
   默认为 true 才碰巧生效。
6. security 客户端侧 key（`ca-file`/`server-name`/`insecure-skip-verify`）绑定了但在
   服务端是死 key；无 mTLS 姿态。
