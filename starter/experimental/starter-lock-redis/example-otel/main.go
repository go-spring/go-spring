...
// Package main is the observability example for starter-lock-redis.
//
// It exercises Redis-backed distributed locks (TryAcquire, Acquire, Unlock)
// through an OTLP-instrumented client, verifies lock spans reach Jaeger,
...
func runTest(s *Service) {
	...
	// Wait for collector to flush traces.
	time.Sleep(3 * time.Second)

	// Verify traces appear in Jaeger.
	traces, err := httpGet("http://127.0.0.1:16686/api/traces?service=lock-redis-otel-example&limit=1")
	if err != nil {
		fmt.Fprintln(os.Stderr, "Jaeger API request failed:", err)
		os.Exit(1)
	}
	if !strings.Contains(traces, `"data":[`) {
		fmt.Fprintln(os.Stderr, "no traces found in Jaeger for service 'lock-redis-otel-example'")
		os.Exit(1)
	}
	fmt.Println("OK: traces found in Jaeger for service 'lock-redis-otel-example'")
