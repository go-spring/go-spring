...
（`starter.go:64-69`，`gs.OnProperty` 前缀判断）；每个 discovery 后端设置其
`spring.discovery.nacos.<name>.server` 才存在。没有任何 `enabled` 开关。
...
require (
    ...
    go-spring.org/starter-registry-nacos     latest   // experimental 路径，模块名见 README
...
gs.Run()
  ...
  └─ SIGTERM：PreStop → 先 Deregister（早于 pre-stop 延迟、早于任何 server 停止
       ...
       Stop/StopContext 兜底再反注册一次；反注册幂等
```
...
| Key | 类型 | 默认值 | 行为 | 配错后果 |
|-----|------|--------|------|----------|
| `server` | string | — | **必填**，`expr:"$ != ''"` 校验。`<name>` 重复 → 启动报 "already registered"。 | 缺失 → 绑定期报错。 |
...
后端是全局 `cloud/discovery` 注册表里的命名适配器，不是 bean：client starter 按名引用
（gateway：`spring.gateway.discovery` / 路由级 `upstream.discovery`）。
