...
[starter/DESIGN.md](../../DESIGN.md) §2.3),把 `cloud/experimental/transaction` 中的 Saga
...
- **默认内存 Store,持久化 Store 通过 `OnMissingBean` 让位。**starter 用
  ...
  崩溃恢复。
- **`Observer` 缝隙做 tracing。**`cloud/experimental/transaction` 是零依赖,不能 import
...
- **重试策略复用 `cloud/governance/resilience`。**`RetryPolicy = resilience.Policy`
...
- **`cloud/experimental/transaction` 依赖 otel——弃选。**`Observer` 缝隙保留零依赖
  不变量。
