...
// Package main is the observability example for starter-oauth2-client.
//
// It exercises an OAuth2 client-credentials backed HTTP client against in-process
// auth and resource servers, verifies OAuth2 flows (token fetch, caching, auth
// code URL) and resilience retries, then checks that client spans reach Jaeger.
...
func runTest(s *Service) {
	...
	traces, err := httpGet("http://127.0.0.1:16686/api/traces?service=oauth2-client-otel-example&limit=1")
	if err != nil {
		fmt.Fprintln(os.Stderr, "Jaeger API request failed:", err)
		os.Exit(1)
	}
	if !strings.Contains(traces, `"data":[`) {
		fmt.Fprintln(os.Stderr, "no traces found in Jaeger for service 'oauth2-client-otel-example'")
		os.Exit(1)
	}
	fmt.Println("OK: traces found in Jaeger for service 'oauth2-client-otel-example'")
