...
**Activation**: any `spring.lock.instances.consul.<name>.*` property registers one Consul-backed `lock.Locker`
...
func (w *Worker) Init(ctx context.Context) {
    ...
    e := lock.NewElection(lock.ElectionConfig{
        ...
        OnStartedLeading: func(context.Context) { log.Infof(ctx, log.TagAppDef, "elected leader") },
    })
    go func() { _ = e.Run(ctx) }()
}
...
spring.lock.instances.consul.jobs.ttl=15s
# Session TTL must sit in Consul's [10s, 86400s] window; out-of-range values
# are clamped per acquisition, not rejected.
spring.lock.instances.consul.jobs.key-prefix=demo/jobs/

# --- lock: singleton worker --------------------------------------------------
spring.lock.instances.consul.singleton.address=127.0.0.1:8500
spring.lock.instances.consul.singleton.key-prefix=demo/singleton/
...
import starter-lock-consul
  ...
  ├─ config bind: ${spring.lock.instances.consul.<name>} → Config (value tags)
...
| 2. starter default | `spring.lock.instances.consul.<name>.ttl` etc. | **TTL only** — consul auto-renews the session and blocks internally in its own acquire loop, so no renew/retry keys exist |
...
All keys live under `spring.lock.instances.consul.<name>` (exact-match, no relaxed forms).

| Key | Type | Default | Behavior / interactions | Misconfiguration consequence |
|-----|------|---------|-------------------------|------------------------------|
| `address` | string | — | **Required.** Consul agent endpoint, e.g. `127.0.0.1:8500`. Checked before bean registration. | Empty → boot fails naming the instance (`lock-consul: spring.lock.instances.consul.<n>.address is required`). |
...
| `tls.enabled` | bool | `false` | Applies the shared `security` block (`server-name`, `ca-file`, `cert-file`, `key-file`, `insecure-skip-verify`) to the API client. | Enabled without material → client-creation error at boot (fail fast). |
...
1. Configure a short TTL: `spring.lock.instances.consul.jobs.ttl=10s` (the clamp floor).
...
|---------|--------------|-----|
| Boot fails `spring.lock.instances.consul.<n>.address is required` | instance without address | Set the key or drop the instance. |
| Boot fails at client creation / TLS | bad `tls.*` material or unreachable scheme | Fix certs; creation is fail-fast. |
| `Acquire` returns 403-ish ACL error | wrong/missing `token` | ACL is checked on first use, not at boot — set `spring.lock.instances.consul.<n>.token`. |
