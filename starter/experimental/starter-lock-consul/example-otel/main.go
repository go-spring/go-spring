...
// Package main is the observability example for starter-lock-consul.
//
// It runs Consul-backed distributed lock operations (TryAcquire, Acquire,
// Unlock) through an OTel-instrumented locker, verifies client spans reach
// Jaeger, then self-exits. Traces are exported via OTLP/gRPC to Jaeger
// (docker-compose).
//
...
func runTest(s *Service) {
	...
	traces, err := httpGet("http://127.0.0.1:16686/api/traces?service=lock-consul-otel-example&limit=1")
	if err != nil {
		fmt.Fprintln(os.Stderr, "Jaeger API request failed:", err)
		os.Exit(1)
	}
	if !strings.Contains(traces, `"data":[`) {
		fmt.Fprintln(os.Stderr, "no traces found in Jaeger for service 'lock-consul-otel-example'")
		os.Exit(1)
	}
	fmt.Println("OK: traces found in Jaeger for service 'lock-consul-otel-example'")
