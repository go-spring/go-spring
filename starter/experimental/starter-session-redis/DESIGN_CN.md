...
[starter/DESIGN.md](../../DESIGN.md) §2.3),为 `cloud/session`(Spring Session
的 Go-Spring 等价物)提供基于 Redis 的 `session.SessionStore`。不开端口;
贡献一个 `SessionStore` bean,由 `cloud/session.Manager` 用于跨副本加载
与保存会话。
...
`cloud/session` 分三段拆分,让 starter 无需触碰 HTTP 就能接入:
...
- **走 bean,而非 driver 注册表。**Session 需要应用已装配的**活** redis
  ...
  `cloud/session` 的 driver 注册表只服务静态默认(`"memory"`);Redis 走
...
## 4. Manager 决策(上下文,住在 `cloud/session`,此处镜像)
