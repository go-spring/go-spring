...
[starter/DESIGN.md](../../DESIGN.md) §2.3),为 `cloud/experimental/session`(Spring Session
的 Go-Spring 等价物)提供基于 Redis 的 `session.SessionStore`。不开端口;
贡献一个 `SessionStore` bean,由 `cloud/experimental/session.Manager` 用于跨副本加载
与保存会话。
...
- **不在范围内:**HTTP `Manager` 中间件与 cookie 处理(那是
  `cloud/experimental/session` 的事);redis 客户端本身(那是 `starter-go-redis`)。

## 2. 关键抽象与缝隙

`cloud/experimental/session` 分三段拆分,让 starter 无需触碰 HTTP 就能接入:
...
- **走 bean,而非注册表。**Session 需要应用已装配的**活** redis
  客户端;把活连接注册进包级 registry 对多 run / 多 test 场景是错的。
  `cloud/experimental/session` 根本不留 store 注册表——store 经构造器注入到
  `Manager`,在这里就是一个 bean,走 `gs.Module` + `TagArg(client)`。
- **多实例经 `gs.Module(OnProperty("spring.session.redis.instances"))`。**
  每个 `spring.session.redis.instances.<name>` 条目按名注入 redis bean。
  `client` 为空时构造 fail-fast;不做 `localhost` 静默兜底(家族规则 §2.2)。
...
## 4. Manager 决策(上下文,住在 `cloud/experimental/session`,此处镜像)
...
- **全局注册表存活连接——弃选。**活连接进包级 map 会破坏 test / run 隔离。
- **按实现拆前缀——弃选。**遵循家族共享前缀规则(`spring.session`);把
  这个 Redis 后端换成另一个贡献同一 `SessionStore` 接口的后端只需换
  blank-import,其它一切不动。内置的进程内 `Memory` store 贡献方式相同
  ——`cloud/experimental/session/example` 就用一个本地 starter 在
  `spring.session.memory.instances` 下把它接进来。
