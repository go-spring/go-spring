...
除原生连接外,本 starter 还可暴露一个 broker 中立的 `messaging.Driver`
(来自 `go-spring.org/cloud/messaging`),让业务代码收发 `*messaging.Message`
信封而不依赖 `amqp` API —— 底层换 broker 时业务代码无需改动。

从 `*amqp.Connection` 注册一个 driver bean(用 `gs.TagArg` 选取具名实例):
...
`*amqp.Connection` bean 仍可用于自定义 exchange、路由、publisher confirm 等 driver 未建模
的 AMQP 能力。

## 高级功能

* **多 RabbitMQ 实例**：`spring.rabbitmq` 下的每一项都会成为一个独立配置的
