...
// Package main is the load-test binary for starter-mongodb. It wires a MongoDB
// client (resilience + fault hot-reloadable) and drives an upsert + findOne
...
type Service struct {
	Mongo *StarterMongoDB.Client `autowire:"load"`
}
...
func runLoad(s *Service) {
	...
	coll := s.Mongo.Database("load").Collection("kv")
	filter := bson.M{"_id": "load"}
	op := func(ctx context.Context) error {
		if _, err := coll.UpdateOne(ctx, filter, bson.M{"$set": bson.M{"v": "v"}},
			options.UpdateOne().SetUpsert(true)); err != nil {
			return err
		}
		var res bson.M
		return coll.FindOne(ctx, filter).Decode(&res)
	}
