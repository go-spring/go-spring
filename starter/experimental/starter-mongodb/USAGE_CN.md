...
gs.Run()
  ...
  │   → newLiveResolver（设置 service-name 且非 mesh 时起发现 watch）[starter.go:121]
...
- **观测走 command monitor**（[command.go:53]）：`event.CommandMonitor` 的
  ...
  monitor 经 atomic pointer 惰性读 observer，因为它在构造期安装、早于 gs 字段注入
  `Observability`；nil 保护覆盖启动 ping 路径（[command.go:41-45]）。
...
这里的桥接把三信号委托给共享 observe kit，使 MongoDB 与其他 client starter 同一套
词汇（[command.go:47-52]）。
...
2. 池中无空闲连接时，驱动调 `dialerWrapper.DialContext` → resilience executor 申请配额
   ...
   操作浮出 `resilience.ErrRateLimited`。设置 service-name 时，底层拨号先经发现
   Resolver 选活端点、忽略 URI 地址（[starter.go:130-137]）。
...
4. 应答触发 `Succeeded`（或 `Failed`）：span 收尾、记录 `db.client.operation.duration`、
   平衡 in-flight gauge，并以 Info 级输出一条 `_app_mongodb_access` 日志（失败为 Warn +
   `error` 字段）。
...
实例 key 位于 `spring.mongodb.<name>.`（经 `conf.BindEach` 绑定）。例外：
observability 块是**顶层** key（见 §3.3 ⚠）。
...
| `service-name` | string | — | 经已注册的发现后端解析地址；每次建连由 Resolver 拨号替代 URI host [starter.go:126-137]。⚠ **绕过 MongoDB 自身拓扑发现**（副本集/mongos）——驱动拨命名服务给出的地址；需在 URI 配合 `directConnection=true`（[config.go:80-84]）。mesh 模式下忽略（sidecar 负责发现+LB）。 | 副本集 URI 不加 `directConnection=true` → "no such host"/拓扑报错；占位 URI 只有在 resolver 真被咨询时才能证明发现生效。 |
| `scheme` | string | — | 把发现端点收窄到一种传输 scheme（如 `tls`）。仅 service-name 生效时被咨询。 | — |
| `discovery` | string | `default` | 用哪个已注册发现后端解析 service-name。 | 未注册后端 → discovery.NewResolver 启动报错。 |
...
| Key | 类型 | 默认值 | 行为 / 联动 | 配错后果 |
|-----|------|--------|-------------|----------|
| `observability.level` | string | `brief` | 访问日志：`off` / `brief` / `detailed`（detailed 追加参数 = 库名）。⚠ 它由 `Client.Observability` 以 `${observability:=}` 绑定——是**顶层绝对属性**（`observability.level=...`），不是 `spring.mongodb.<name>.observability.level`；所有实例共享一份。 | 配在实例前缀下 → 静默无效，日志保持 `brief`。 |
| `observability.maxArgBytes` | int | `512` | detailed 模式参数捕获上限。⚠ 同顶层规则。 | — |
| `observability.skipOps` | list | — | 对列出的命令名（`find`、`insert`、`ping`……）同时抑制 span + metric + log。⚠ 同顶层规则。 | — |

trace 与 metric 本身在此**没有实例级开关**：它们搭乘 starter-otel 安装的 OTel 全局
（`spring.observability.*`）；无 starter-otel 时为 no-op。
...
# brief：db.operation=find status=ok duration_ms=1.2 ；失败时追加 error=... 并升为 Warn
```

- span：以 MongoDB 命令命名（`find`、`insert`、`ping`），属性 `db.system=mongodb`、
  `db.operation=<命令>`；detailed 模式下 `db.statement=<库名>`。
...
扩缩/迁移后端实例；每条**新**连接都向 Resolver 要活端点。配 `max-conn-idle-time=5m`，
...
| `observability.level=detailed` 不生效 | key 配在实例前缀下；绑定在顶层 | 顶层写 `observability.level=detailed`（§3.3 ⚠）。 |
| 访问日志太吵（每条 `ping`/`find`） | brief 级每条命令都记 | `observability.skipOps=ping,hello` 或 `observability.level=off`（仅日志）。 |
...
| 指标 | 数值 |
|------|------|
| 配置 key 总数 | 14 个实例 key + tls 组 + 3 个顶层 observability |
| 其中必填 | 1（`uri`） |
| quickstart 前置外部依赖 | 1（MongoDB） |
| "注意/坑" 条数 | 6（directConnection、顶层 observability 计 1 条、焐热池绕过、username 门控凭据） |
...
- 实例级 `observability` 实际绑定为**顶层** key、所有实例共享，而非实例前缀下——
  反直觉且 config.go 未记载。
- resilience 仅建连层；期待逐命令 breaker 语义（如 starter-go-redis）的用户对焐热池
  ...
  可拒绝的 hook 再升级。
- （NEW）command.go 桥接 observe kit 时 span 参数只取库名；命令文档（filter/update）
  即使 detailed 也不捕获——刻意为之（载荷安全），但使 MongoDB 的 `detailed` 与
  `brief` 几乎无差别。
