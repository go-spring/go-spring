...
**激活方式**：任一 `spring.lock.instances.k8s.<name>.*` 配置即为每个 `<name>` 注册一个 Lease 后端的
...
所有 key 位于 `spring.lock.instances.k8s.<name>` 之下（精确匹配，无宽松形态）。
...
| 症状 | 可能原因 | 处置 |
|------|----------|------|
| 启动报 `in-cluster config` | 集群外运行且未设 `kubeconfig` | 设置 `spring.lock.instances.k8s.<n>.kubeconfig`。 |
