...
**Activation**: any `spring.lock.instances.k8s.<name>.*` property registers one Lease-backed `lock.Locker`
...
All keys live under `spring.lock.instances.k8s.<name>` (exact-match, no relaxed forms).
...
|---------|--------------|-----|
| Boot fails `in-cluster config` | running outside a cluster without `kubeconfig` | Set `spring.lock.instances.k8s.<n>.kubeconfig`. |
