...
// k8s Lease locker and — by default — wraps it with the shared observe-lock
// adapter (trace span + duration/in-flight metric + access log;
// lock.system="k8s"), so the primary bean is transparently observed.
...
func wrapIfObserved(c Config, inner lock.Locker) lock.Locker {
	...
	return lockobserve.WrapLocker("k8s", c.Observability, inner)
}
