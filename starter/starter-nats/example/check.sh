#!/usr/bin/env bash
#
# Smoke test for starter-nats. Brings up a local NATS server (with JetStream
# enabled via -js) through docker compose, runs the example (which self-asserts
...
# Wait for NATS to accept client connections (up to 60s).
