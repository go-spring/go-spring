...
内部的每次尝试超时，且它自身不打开 span。

### 2.5 健康检查

除非条目设置 `health.enabled=false`，每个实例都会贡献一个名为 `nats:<name>` 的
`health.Indicator` [starter.go]。探针读取 `Conn.IsConnected()`，因此重连间隙中的实例会
报 down 并在重连后自行恢复。对应用离开它就无法服务的依赖而言这是正确默认；若某条连接
...
- **未解**：JetStream 消费无 trace（独立 API 面）；多值 header 压平取首值；
  `RequestGuarded` 完全不打开 span；guarded 方法无 observe 插桩。
