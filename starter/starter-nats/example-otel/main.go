...
// Package main is the observability example for starter-nats.
//
// It runs NATS core pub/sub, request-reply, queue group, and JetStream
// operations through an OTel-instrumented connection, verifies trace spans
// reach Jaeger, then self-exits. Traces are exported via OTLP/gRPC to
...
func runTest(s *Service) {
	...
	// instrumented entries: PublishMsgContext emits the producer span and injects
	// trace context into the message header, and the Consume handler receives a
	// ctx carrying it, so the consumer span links back to the producer (both
	// no-ops unless starter-otel is imported).
	{
		received := make(chan string, 1)
		sub, err := s.Main.Consume(context.Background(), "demo.pubsub", "",
		...
		_ = s.Main.Flush()
		err = s.Main.PublishMsgContext(context.Background(),
			&nats.Msg{Subject: "demo.pubsub", Data: []byte("hello")})
		if err != nil {
	...
	traces, err := httpGet("http://127.0.0.1:16686/api/traces?service=nats-otel-example&limit=1")
	if err != nil {
		fmt.Fprintln(os.Stderr, "Jaeger API request failed:", err)
		os.Exit(1)
	}
	if !strings.Contains(traces, `"data":[`) {
		fmt.Fprintln(os.Stderr, "no traces found in Jaeger for service 'nats-otel-example'")
		os.Exit(1)
	}
	fmt.Println("OK: traces found in Jaeger for service 'nats-otel-example'")
