...
`messaging` 是与 broker 无关的发布/订阅抽象——Spring Cloud Stream binder 模型
的 Go 惯用法等价物。业务代码通过统一的 `Publisher` / `Subscriber` 收发
`Message` 信封,换 broker(NATS、Kafka、Pulsar、RabbitMQ、MQTT、...)只改接
线,不改业务代码。
...
| `Binder` | 在一条 broker 连接上开 publisher/subscriber;destination/source/group 字符串**按 broker 自己的术语**解释(subject、topic、queue、consumer group)。 |
...
| `RegisterBinder` / `GetBinder` | 驱动注册表惯用法(空名/nil/重复即 panic),面向按配置名选进程级 binder 的调用方。starter 通常把 binder 作为 bean 接在活连接上。 |
...
| `NewMessageID()` / `HeaderMessageID` | 生成 32 位 hex id + 它所属的保留 header。至少一次投递下消费端去重是生产必做;以这个 id 为键。binder 的 publisher 侧经 `EnsureMessageID` 缺失即自动盖章——自己设了(或用业务键去重)它就是 no-op。 |
| `HeaderDeliveryAttempt` | 保留 header,binder 消费侧从 broker 重投计数填入(1 起;缺失或 "1" = 首次投递)。读它可以决定"重试"还是"直接进死信",不依赖进程内状态(重启即失)。 |
| `BatchPublish(ctx, p, msgs...)` | 一次调用批量发送。publisher 实现了可选的 `BatchPublisher` 能力则走批量,否则按序逐条发,遇第一个错误停止(非事务)。 |

实现 `Binder` 的 starter:`starter-nats`、`starter-kafka`、
...
```go
// binder 的 publisher 缺失时已自动盖 message-id;只有自己持有 id 才需要设:
...
- **不做批量消费。** `Publish` 一次一条(发送侧可用 `BatchPublish` 批量);
  消费要吞吐,由 binder 内部批量、对外仍是逐条 Handler。
