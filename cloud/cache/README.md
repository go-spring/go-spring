...
- This package is container-free. The gs wiring — the driver registry
  (`RegisterDriver`/`GetDriver`, mirroring the discovery and resilience driver
  idioms) and the config-driven module — lives in **starter-cache**; under
  `spring.cache`, each entry's `driver = "<driver>:<beanID>"` selects a
  registered driver and the backend bean it wraps. Backend starters (bigcache,
  memcached, redigo, go-redis) register their drivers there, so importing one
  of them is what activates the wiring.
