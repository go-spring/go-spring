...
//
// [Listener] is the declarative seam for container-driven subscription: a bean
// exported as a Listener can wire its handlers in one place. The container-side
// wiring (collecting Listener beans and calling [Listener.Register]) is not yet
// part of this repository; see the example module under ./example for how an
// application assembles it today.
package event
...
// use: multiple goroutines may publish and subscribe at once. Create one with
// [New]; there is deliberately no Bus interface to implement or wrap.
type Bus struct {
	mu     sync.RWMutex
	seq    uint64
...
// asyncEvent carries a published value and its context to an async worker. The
// context is detached from the publishing call (see detach), and cancel stops
// its timeout timer once the event has been delivered.
...
func (b *Bus) Publish(ctx context.Context, event any) error {
	...
	for _, e := range snapshot {
		if e.async {
			// Async handlers run later, on their own goroutine: detaching the
			// context keeps the publisher's cancellation (or its deadline
			// expiring after enqueue) from aborting a handler that already
			// accepted the event.
			if asyncCtx == nil {
				asyncCtx, asyncCancel = detach(ctx)
			}
			select {
			case e.ch <- asyncEvent{ctx: asyncCtx, cancel: asyncCancel, event: event}:
			case <-e.done:
...
// detach returns a context that is not cancelled when the publishing call is
// cancelled, so an already-enqueued async handler always runs to completion. A
// parent deadline is preserved as a fresh timeout with the remaining budget:
// [context.WithoutCancel] alone would carry the deadline as inert data that can
// never fire, which would surprise handlers that expect it to be enforced. The
// returned cancel stops that timer and must be called once delivery is done.
func detach(ctx context.Context) (context.Context, context.CancelFunc) {
	ctx = context.WithoutCancel(ctx)
	if dl, ok := ctx.Deadline(); ok {
		return context.WithTimeout(ctx, time.Until(dl))
	}
	return ctx, func() {}
}
...
// and is idempotent.
//
// The handler receives a context detached from the publishing call: cancelling
// the context passed to [Bus.Publish] (including its deadline expiring after
// enqueue) never aborts an already-enqueued async handler.
...
func subscribe[T any](bus *Bus, handler func(ctx context.Context, event T) error, async bool, opts []SubOption) func() {
	...
	return bus.subscribe(reflect.TypeFor[T](), invoke, async, o)
}
