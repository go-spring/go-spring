...
import (
	...
	"go-spring.org/cloud/experimental/event"
...
// TestAsyncHandlerRunsToCompletionAfterPublishContextCancelled verifies the
// async ctx fix: cancelling the context passed to Publish after the event has
// been enqueued never aborts the async handler — it still receives a live
// context and runs to completion.
func TestAsyncHandlerRunsToCompletionAfterPublishContextCancelled(t *testing.T) {
	bus := event.New()
	defer bus.Close()

	done := make(chan error, 1)
	event.SubscribeAsync(bus, func(ctx context.Context, e eventA) error {
		// The publish call is already cancelled by the time this runs; the
		// handler must not observe that cancellation.
		select {
		case <-ctx.Done():
			done <- ctx.Err()
	...
	ctx, cancel := context.WithCancel(context.Background())
	assert.Error(t, bus.Publish(ctx, eventA{})).Nil()
	cancel()
