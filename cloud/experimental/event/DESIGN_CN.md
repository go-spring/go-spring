...
- `Bus` 是具体结构体(刻意不提供 `Bus` 接口供实现或包装),只有
  `Publish(ctx, any) error` 与 `Close() error`;泛型自由函数 `Subscribe[T]` /
  `SubscribeAsync[T]` 直接收 `*Bus`,保调用点类型安全、零反射,也不存在
  断言失败被静默吞掉的路径。
- `Listener` 是容器侧的非泛型收集缝隙:bean 通过 Export 为 `event.Listener`
  收集(与 `health.Indicator` 的 Export 收集范式同构),`Register(bus)` 内部
  再调用泛型 `Subscribe[T]`。容器侧的注册器尚未入库,由 example 手工装配。
...
- **异步 worker 通道**:发送用 `select { ch<- / done / ctx.Done() }` 三路;
  ...
  不会泄漏 publisher。select 里的 ctx 是发布者自己的 ctx(背压时中止发送);
  交给处理器的 ctx 则先做了分离(`context.WithoutCancel`,有 deadline 再包
  剩余时间的 `WithTimeout`),已入队的事件不会被发布调用的取消所中止。
