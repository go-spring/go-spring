...
期)在 [example/](example/),它是一个把 store 作为 bean 注入的 Go-Spring
应用。
...
store 由 starter 以 bean 形式贡献;应用在装配 handler 的地方构造 `Manager`:
...
`starter-session-redis` 按 `spring.session.redis.instances.<name>` 贡献这个
`session.SessionStore`。进程内 store 的贡献方式相同:[example/](example/) 在
[starter.go](example/starter.go) 里定义了一个本地 starter,按
`spring.session.memory.instances.<name>` 贡献同一个接口——两个后端之间只差一
个空白导入和一个配置 key,handler 代码不变。

### 长驻进程里的过期回收

`Memory` 在会话被再次读取时把它当作不存在并顺手删除。因此一个过期后再也没
被读取的会话会在 map 里留到进程结束——分布式后端靠 key TTL 天然没有这个问
题。`Memory.StartCleanup(interval)` 用后台清理覆盖这种情况,并配 `Stop`。示
例里的本地 starter 把间隔暴露成 `clean-interval`,在 bean 的 destroy 钩子里
停掉清理协程;非正的间隔(默认值)只保留惰性路径。
