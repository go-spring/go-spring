...
- **`Store` interface as the backend seam.** There is no global driver registry.
  ...
  seam is the interface a starter implements, the same choice `cloud/experimental/batch` and
  `cloud/experimental/lock` make. The `Store` owns `EnsureVersionTable`, `AppliedRecords`,
