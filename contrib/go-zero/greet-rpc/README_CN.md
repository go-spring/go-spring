...
├── scripts/gen-code.sh     # 基于 proto/greet.proto 通过 goctl 重新生成 proto/ 桩代码
├── proto/greet.pb.go       # protoc 生成的消息(请勿手改)
├── proto/greet_grpc.pb.go  # protoc 生成的 gRPC 桩代码(请勿手改)
...
# 从 IDL 生成 proto/ 桩代码(或直接执行 ./scripts/gen-code.sh)
...
托管生命周期与配置的部分;本示例把它整个丢弃,只保留 `proto/` 产物 —— 生命周期
与配置交由 Go-Spring 管理。`scripts/gen-code.sh` 把 `--zrpc_out` 指向一个 `mktemp -d`
...
| handler  | `server.RegisterGreetServer(srv, logic)`   | `gs.Provide(func() ServiceRegister { return greet.RegisterGreetServer(...) })`      |
