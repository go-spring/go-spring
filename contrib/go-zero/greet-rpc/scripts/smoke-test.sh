#!/usr/bin/env bash
#
# Smoke test for the go-zero zrpc registry example. Brings up a local etcd via
# docker compose, starts the provider (which registers greet.rpc into etcd),
...
# Start the provider; it registers into etcd and serves on :8081. Build to a
...
# Wait for the provider's zrpc port to accept connections (up to 30s).
for _ in $(seq 1 30); do
