...
// log line into go-spring's log module. This is part of the wider bridge goal:
// let each contrib framework's internal logs flow through go-spring's log so
// users only configure one logging pipeline (here the root FileLogger in
// conf/app.properties, which ships to Loki). zrpc transport start/stop, etcd
// registration and interceptor errors thus land in provider.log as JSON, next
// to the business logs.
//
...
// However, go-zero injects trace/span into `fields` via logx.WithContext, so
// correlation still travels through — we just forward every LogField as a
// structured field. The caller (file:line) recorded by go-spring points into
// this bridge rather than the real emit site; we accept that imprecision (see
// the log-bridge design discussion) because logx buries the real caller behind
// several internal frames anyway.
type gsBridgeWriter struct {
	tag *log.Tag
}

// newGSBridgeWriter builds the bridge, tagging every forwarded line as an RPC
// log under "rpc.gozero" so it can be filtered or routed to a dedicated logger
// later without touching the framework wiring.
func newGSBridgeWriter() logx.Writer {
...
// FileLogger appender, not by this bridge, so there is nothing to flush here.
