...
| 启动        | `RestServer` 实现 `gs.Server`；`gs.Run()` 驱动 | `RpcServer` 实现 `gs.Server`；`gs.Run()` 驱动 | 与 greet-api 完全一致 —— 共用同一 starter 适配器                           |

适配器代码位于 `starter-go-zero/rest`，因此与 `greet-api` 逐字共用:
...
# go-zero rest.Server 设置，由 starter-go-zero/rest 经 ${spring.go-zero.rest.server} 前缀读取。
...
代码** —— starter 的 `RestServer` 只是从 `conf/app.properties` 里读取字段填进
`ServiceConf`。
...
日志已经不再落到 go-zero 自己的 `.log` 文件里：starter（`starter-go-zero/rest`）
