...
| handler  | `RegisterGreetServiceHandler(srv, &impl)`  | `StarterDubbo.RegisterService(...)`,服务无关地绑定（+ per-service/method 配置）|
| 是否启用 | 总是开启                                   | 通过 `gs.OnBean` 条件依赖 `ServiceRegister` bean                          |
| 端口     | 写死的默认值                               | 取自 `conf/app.properties` 的 `${spring.dubbo.protocols.tri.port}` |
...
加顶层 `${spring.dubbo.registries}` 构建)提供;consumer 按类型 autowire 它,
再用 Triple 生成的桩发起调用。若要运行两个同类型注册中心,给各自一个不同的
