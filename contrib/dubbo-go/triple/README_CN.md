...
├── provider/handler.go      # GreetProvider,通过 ServiceRegister bean 装配
...
| handler  | `RegisterGreetServiceHandler(srv, &impl)`  | `gs.Provide(func() ServiceRegister { ... })`,服务无关地绑定              |
...
`provider/server.go` 里的适配器是关键:Dubbo-go 的 `Serve()` 会绑定监听端口、
