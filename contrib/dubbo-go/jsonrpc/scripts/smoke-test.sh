#!/usr/bin/env bash
#
# Smoke test for the dubbo-go JSON-RPC example, extended to exercise
...
# provider (registers com.example.GreetService into etcd, exposes Prometheus
# metrics on :9090, exports traces to Jaeger, writes JSON logs to logs/), runs
# the consumer (discovers the provider, calls it, and prints the echoed value),
...
# provider's /metrics endpoint serves dubbo metrics, the RPC round-trips, and the
# backend containers stay up. Confirming the data is actually queryable in
...
# Start the provider; it registers into etcd, serves JSON-RPC on :20002 and
...
wait_tcp 127.0.0.1 20002 || { echo "provider jsonrpc port not ready"; exit 1; }
...
# needs no env-var overrides. The consumer makes one canonical call plus a batch
# of further calls, so the observability backends receive real volume (many
# spans, climbing counters, several log lines) rather than a single sample.
# Capture stdout to assert on the echoed value — the consumer runs via gs.Run,
# which exits 0 even if wiring fails, so the exit code alone is not a reliable
# smoke signal.
consumer_out="$(mktemp)"
...
# per-method request counter is served locally on /metrics and is cumulative, so
# after the consumer finishes it reflects every call (canonical + batch). We
# assert it climbed well past 1 — proof the enriched load reached the provider
# and is being counted (this is provider-local and deterministic; whether it
# then landed in Prometheus/Jaeger/Loki is the README's manual step).
if command -v curl >/dev/null 2>&1; then
