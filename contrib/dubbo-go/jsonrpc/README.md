...
transport with a **JSON** body — refactored to boot and be configured the
Go-Spring way: `gs.Run()` drives the lifecycle, the provider is an IoC bean,
and the bind port comes from `conf/app.properties` instead of hard-coded
`main()` wiring.
...
# JSON-RPC bind port; read via the ${spring.dubbo.server} prefix, default 20002
# (20000/20001 are reserved for the Triple/Dubbo siblings so all three can
# coexist on one host).
