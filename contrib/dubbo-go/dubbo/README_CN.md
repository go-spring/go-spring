...
├── observability/           # 后端栈的 Prometheus 与 Promtail 配置
...
| Traces  | dubbo-go OTel → OTLP/gRPC `127.0.0.1:4317`   | Jaeger          | UI http://127.0.0.1:16686(服务 `dubbo-demo`)     |
...
宿主机上,`observability/prometheus.yml` 把目标写成 `host.docker.internal:9090`
...
`/var/log/app`(docker-compose.yml)。Promtail(`observability/promtail-config.yml`)
