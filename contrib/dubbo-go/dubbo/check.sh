#!/usr/bin/env bash
#
# Smoke test for the dubbo-go classic (Hessian2) registry example. Brings up a
# local etcd via docker compose, starts the provider (which registers
# com.example.GreetService into etcd), runs the consumer (which discovers the
# provider through etcd, calls it and self-asserts, exiting non-zero on
...
# Start the provider; it registers into etcd and serves on :20001.
...
for _ in $(seq 1 30); do
    if (exec 3<>/dev/tcp/127.0.0.1/20001) 2>/dev/null; then
        exec 3>&- 3<&- 2>/dev/null || true
        break
    fi
    sleep 1
