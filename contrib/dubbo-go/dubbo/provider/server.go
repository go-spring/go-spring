...
func init() {
	...
	// service-agnostic — the same adapter shape used by the Triple sibling.
...
// The port defaults to 20001 to avoid colliding with the Triple sibling that
// defaults to 20000 when the two are run side by side.
type Config struct {
	Port         int    `value:"${port:=20001}"`
...
// lifecycle. The protocol here is the classic Dubbo protocol — TCP transport
// with Hessian2 serialization — selected via protocol.WithDubbo(). This is
// the interop path for talking to Java Dubbo services; use the Triple sibling
// for greenfield Go microservices.
...
// Run builds the Dubbo classic (Hessian2) server on the configured port and
// starts serving once Go-Spring signals readiness. Serving with a registry
// configured makes Dubbo publish the provider's address into etcd under its
// Java-style interface name (com.example.GreetService), which is what the
// consumer later resolves. Dubbo's Serve blocks forever internally, so it
// runs in a goroutine while Run parks on the done channel; Stop closes done
// to hand control back to Go-Spring.
func (s *DubboServer) Run(ctx context.Context, sig gs.ReadySignal) error {
	svr, err := server.NewServer(
		server.WithServerProtocol(
			...
			// WithDubbo selects the classic Dubbo protocol: TCP transport,
			// Hessian2 wire encoding. This is what distinguishes this module
			// from the Triple sibling (protobuf over HTTP/2).
			protocol.WithDubbo(),
		),
