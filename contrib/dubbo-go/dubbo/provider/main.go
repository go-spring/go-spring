...
// The provider is a long-lived process: gs.Run() starts the Dubbo server
// registered in server.go, which publishes itself into etcd, then blocks
// until it receives SIGTERM/SIGINT. Unlike the old single-process example,
// there is no inline client here — discovery and the RPC call live in the
// consumer.
func main() {
	_ = os.Unsetenv("_")
	_ = os.Unsetenv("TERM")
	...
	// gs.Run() starts only the Dubbo server registered in server.go.
	gs.Run()
}
