#!/usr/bin/env bash
#
# Smoke test for the dubbo-go REST registry example. Brings up a local
...
# Start the provider; it registers into etcd and serves on :20003.
...
for _ in $(seq 1 30); do
    if (exec 3<>/dev/tcp/127.0.0.1/20003) 2>/dev/null; then
        exec 3>&- 3<&- 2>/dev/null || true
        break
    fi
    sleep 1
