...
func init() {
	...
	// service-agnostic — the same adapter shape used by the classic-Dubbo,
	// JSON-RPC and Triple siblings.
...
// The port defaults to 20003 so this REST provider can coexist with the
// Triple (20000), classic-Dubbo (20001) and JSON-RPC (20002) siblings on
// the same host.
type Config struct {
	Port         int    `value:"${port:=20003}"`
...
// lifecycle. The protocol here is REST — HTTP/1.1 transport with per-method
// (verb, path, param-source) routing, backed by go-restful — selected via
// protocol.WithREST(). Unlike its siblings, REST cannot be driven by
// method-reflection alone: dubbo-go needs a RestServiceConfig map (see
// rest_service.go) that pins every method's URL layout before Serve is
// called. Without that map the REST protocol logs an error and refuses
// to export.
...
// Run builds the REST server on the configured port and starts serving once
// Go-Spring signals readiness. Serving with a registry configured makes
...
func (s *DubboServer) Run(ctx context.Context, sig gs.ReadySignal) error {
	svr, err := server.NewServer(
		server.WithServerProtocol(
			...
			// WithREST selects the REST protocol: HTTP/1.1 with the request
			// mapped onto a URL template + query/header/body params, and the
			// response rendered as JSON by go-restful. This is what
			// distinguishes this module from its siblings — no protobuf,
			// no Hessian2, no JSON-RPC envelope, just a plain REST endpoint.
			protocol.WithREST(),
		),
	...
	if err = s.reg(svr); err != nil {
		return errutil.Explain(err, "failed to register rest service")
	}
	s.svr = svr
