...
// Dubbo publish the provider's address into etcd under its interface name
// (greet.GreetService), which is what the consumer later resolves. Dubbo's
// Serve blocks forever internally, so it runs in a goroutine while Run parks on
// the done channel; Stop closes done to hand control back to Go-Spring.
