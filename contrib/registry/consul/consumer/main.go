...
// protobuf-over-TTHeader payload, the default) and gRPC (protobuf over HTTP/2).
// The choice is made per client via client.WithTransportProtocol; the server
// sniffs the connection and dispatches accordingly.
//
// This consumer resolves the provider from Consul by service name and calls it
...
type Consumer struct {
	RegistryAddr string `value:"${spring.kitex.consumer.registry.consul:=127.0.0.1:8500}"`
...
// Consul, performs one Echo and self-asserts, exiting non-zero on any failure.
func call(registryAddr, serviceName, label string, extra ...client.Option) {
	r, err := consul.NewConsulResolver(registryAddr)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[%s] failed to create consul resolver: %v\n", label, err)
		os.Exit(1)
	}
