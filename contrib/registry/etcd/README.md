# etcd registry (Go-Spring style)

[English](README.md) | [中文](README_CN.md)

Service registration & discovery through **etcd**, using a
...
etcd; the consumer resolves a live provider address from that same etcd instead
of dialing a hard-coded `host:port`.
...
├── consumer/main.go         # discovers via etcd, calls, asserts, exits
├── consumer/conf/app.properties  # consumer config (registry + client protocol)
├── docker-compose.yml       # local etcd
└── scripts/smoke-test.sh    # smoke test: up etcd+provider, run consumer, tear down
```
...
go run ./provider &           # long-lived, registers into etcd
go run ./consumer             # discovers via etcd and calls
```
