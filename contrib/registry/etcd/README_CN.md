# etcd 注册中心(Go-Spring 风格)

[English](README.md) | [中文](README_CN.md)

通过 **etcd** 实现服务注册与发现,使用
[dubbo-go](https://dubbo.apache.org/en/overview/mannual/golang-sdk/) 的
`GreetService`,走 **Triple** 协议(protobuf-over-HTTP/2,与 gRPC 线兼容)。
provider 启动时把 `greet.GreetService` 注册进 etcd;consumer 不去拨打写死的
`host:port`,而是从同一个 etcd 解析出存活的 provider 地址。
...
go run ./provider &           # 长期运行,注册进 etcd
go run ./consumer             # 经 etcd 发现并调用
```
