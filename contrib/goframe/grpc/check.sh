#!/usr/bin/env bash
#
# Smoke test for the goframe gRPC registry example. Brings up a local etcd via
# docker compose, starts the provider (which registers goframe.grpc.echo into
# etcd), runs the consumer (which discovers the provider through etcd, calls
# it and self-asserts, exiting non-zero on failure), then tears everything
...
# Start the provider; it registers into etcd and serves on :8001. Build to a
