...
gsvc 集成：starter 的适配器围绕 gtcp 生命周期**手写**了
...
├── provider/handler.go           # 提供 starter-goframe/tcp 的 ServiceRegister；bufio 按行回显处理器
├── consumer/main.go              # gsvc.Search → gtcp.NewNetConn 拨号，断言回显后退出
├── conf/app.properties           # Provider 配置（${spring.goframe.tcp.server}）
...
被显式暴露在 `starter-goframe/tcp` 里：

1. **先绑定，再注册。** 在 `gtcp.Server.Run()` 开始 listen 之前就写 etcd，
   会让消费者拨到一个尚未打开的端口，看到 "connection refused"。starter 把
...
# etcd 注册中心地址；对齐 docker-compose.yml。留空则为普通 TCP server，不注册。
