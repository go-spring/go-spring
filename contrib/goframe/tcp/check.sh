#!/usr/bin/env bash
#
# Smoke test for the goframe raw-TCP registry example. Brings up a local
# etcd via docker compose, starts the provider (which registers
# goframe.tcp.echo into etcd on port 8003), runs the consumer (which
# discovers the provider through etcd, dials tcp://<host>:<port>, sends a
# line-delimited frame and self-asserts on the echo, exiting non-zero on
# failure), then tears everything down. Skipped gracefully when docker is
# unavailable.
#
set -euo pipefail
...
# Start the provider; it registers into etcd and serves on :8003. Build to a
...
# Wait for the provider's TCP port to accept connections (up to 30s).
for _ in $(seq 1 30); do
    if (exec 3<>/dev/tcp/127.0.0.1/8003) 2>/dev/null; then
        exec 3>&- 3<&- 2>/dev/null || true
        break
    fi
    sleep 1
