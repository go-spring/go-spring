#!/usr/bin/env bash
#
# Smoke test for the goframe WebSocket registry example. Brings up a local
# etcd via docker compose, starts the provider (which registers
# goframe.websocket.echo into etcd on port 8002), runs the consumer (which
# discovers the provider through etcd, dials ws://<host>:<port>/echo, echoes
# a message and self-asserts, exiting non-zero on failure), then tears
...
# Start the provider; it registers into etcd and serves on :8002. Build to a
...
# Wait for the provider's HTTP port to accept connections (up to 30s).
for _ in $(seq 1 30); do
    if (exec 3<>/dev/tcp/127.0.0.1/8002) 2>/dev/null; then
        exec 3>&- 3<&- 2>/dev/null || true
        break
    fi
    sleep 1
