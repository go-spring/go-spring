...
├── provider/handler.go           # 提供 starter-goframe/ws 的 ServiceRegister；/echo 升级 + 帧回显处理器
├── consumer/main.go              # gsvc.Search → gorilla-websocket 拨号，断言回显后退出
├── conf/app.properties           # Provider 配置（${spring.goframe.ws.server}）
...
# etcd 注册中心地址；对齐 docker-compose.yml。留空则为普通 server，客户端直连。
