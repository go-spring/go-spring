...
// Prometheus scrape endpoint, and structured JSON logging via glog. We only
// instrument the provider — the consumer stays a bare client, matching the
// dubbo-go / go-zero examples. Each field carries an explicit default so the
// plain smoke test still runs when no observability backend is listening (the
// trace exporter just fails to connect in the background).
type Config struct {
	...
	// Logging: glog's built-in JSON handler writes one structured line per event
	// into LogDir/LogFile, auto-tagging each with the request's trace-id so logs
	// correlate with the spans exported above. Promtail tails LogDir → Loki.
	LogDir  string `value:"${log.dir:=../logs}"`
	LogFile string `value:"${log.file:=provider.log}"`
}
...
func (s *GoFrameServer) initObservability(cfg Config) {
	...
	// Logging: switch glog's default logger to the built-in JSON handler and
	// point it at LogDir/LogFile. glog auto-injects the request's trace-id into
	// each line (see glog_logger.go doDefaultPrint), so file logs correlate with
	// the spans exported above.
	logger := g.Log()
	logger.SetHandlers(glog.HandlerJson)
	if err := logger.SetPath(cfg.LogDir); err != nil {
		g.Log().Fatalf(context.Background(), "init log path: %+v", err)
	}
