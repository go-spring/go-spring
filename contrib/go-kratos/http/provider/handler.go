...
func init() {
	// Provide the single-argument ServiceRegister bean starter-kratos/http
	// depends on: it binds GreeterService onto the kratos HTTP transport server.
	// The starter's HttpServer only materializes because this bean exists, and it
	// never learns about v1.GreeterHTTPServer — registration is entirely here.
	gs.Provide(func() kratoshttp.ServiceRegister {
		return func(hs *khttp.Server) error {
...
// a deterministic value to assert on over HTTP.
