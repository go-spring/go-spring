#!/usr/bin/env bash
#
# Smoke test for the go-kratos WebSocket registry example. Brings up etcd via
# docker compose, starts the provider (registers the kratos-ws server
# "kratos-ws" into etcd, serves WebSocket on :9002, writes JSON business logs to
# logs/), runs the consumer (dials the WS provider directly, sends a framed
# request, asserts the echo, exits non-zero on failure), then tears everything
...
# Start the provider; it registers into etcd and serves WebSocket on :9002.
...
# Run the consumer; it dials the provider's WebSocket endpoint directly and
# performs a framed round trip. Capture stdout to assert on the echoed value.
consumer_out="$(mktemp)"
...
rm -f "${consumer_out}"
echo "OK: consumer discovered the provider and got the expected WebSocket greeting"
