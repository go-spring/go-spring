...
bean 通过 Go-Spring IoC 容器装配(取代 `google/wire`),服务监听地址来自
`provider/conf/app.properties`(取代 Kratos 的 YAML 配置)。脚手架分层的
`internal/{biz,service,data}` 被收敛掉——对这个 Greeter 而言那几层都是空穿透——
业务逻辑直接落在 `provider/handler.go`,与 dubbo-go 示例扁平的 provider/consumer
结构对齐。
...
```
contrib/go-kratos/
├── idl/helloworld/v1/          # protoc 生成的 gRPC + HTTP stub(请勿编辑)
...
`idl/` stub 原样不动,并把 `internal/{biz,service,data}` 分层(对本 Greeter 是
空穿透)收敛进 `provider/handler.go` 的业务逻辑,再按 provider + consumer
双进程模式重连其余部分。

`idl/helloworld/v1/*.pb.go`、`*_grpc.pb.go`、`*_http.pb.go` 这些 stub 可以通过
运行 `./idl/gen-code.sh` 从 `.proto` 重新生成(内部是对 `kratos proto client` 的薄封装)。
...
| 依赖装配       | `google/wire` `ProviderSet` + 生成的 `wire_gen.go`       | `provider/*.go` 里 `init()` + `gs.Provide`;provider 各文件均为 `package main`             |
...
| 业务逻辑       | 分层 `internal/{biz,service,data}`(usecase → repo)      | 折叠进 `provider/handler.go` 的 `GreeterService.SayHello`(那几层是空穿透)               |
| 是否启用       | 总是开启                                                 | `KratosServer` 通过 `gs.OnBean` 条件依赖 `ServiceRegister` bean                            |
| 配置来源       | `configs/config.yaml` 扫描进 `conf.proto` `Bootstrap`    | `provider/conf/app.properties`,经 `value:"${spring.kratos.http}"` / `${spring.kratos.grpc}` 绑定  |
