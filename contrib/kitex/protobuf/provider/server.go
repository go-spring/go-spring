...
//
// Because the service was generated from a protobuf IDL, this one server
// speaks BOTH protobuf transports on the same port at once: KitexProtobuf
// (Kitex's own protobuf-over-TTHeader payload) and gRPC (protobuf over
// HTTP/2). Kitex sniffs the incoming connection and dispatches accordingly, so
// there is nothing protocol-specific to configure here — the consumer picks
// the wire protocol per call (see consumer/main.go).
