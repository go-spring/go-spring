#!/usr/bin/env bash
#
# Smoke test for the kitex registry example. Brings up a local etcd via docker
# compose, starts the provider (which registers the echo service into etcd),
# runs the consumer (which discovers the provider through etcd, calls it and
# self-asserts, exiting non-zero on failure), then tears everything down.
...
# Start the provider; it registers into etcd and serves on :8888. Build to a
...
# Wait for the provider's Kitex port to accept connections (up to 30s).
for _ in $(seq 1 30); do
