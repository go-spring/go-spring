...
├── idl/echo/...          # Kitex 生成代码(仅 PROVIDER 使用)
...
├── consumer/main.go         # 不 import idl/echo;JSON 泛化调用
...
仅 **provider** 埋点,泛化 JSON consumer 保持裸客户端。埋点位于标准 server 侧,
所以无论客户端是否用泛化调用,provider 的观测都一致。OTel meter 已关闭,
指标只走 Prometheus(不重复采集)。

`docker-compose.yml` 会同时起 etcd 与 Jaeger、Prometheus、Loki、Promtail。
跑完 provider + consumer(或冒烟脚本)后逐一核对:

- **Trace** —— Jaeger UI <http://127.0.0.1:16686>,服务 `echo-generic`,查看 `Echo` span。
