...
type Config struct {
	Addr         string `value:"${addr:=:8890}"`
	ServiceName  string `value:"${service.name:=echo-generic}"`
...
//
// This provider is deliberately identical to the ../thrift sibling — a normal
// code-generated Kitex Thrift server. The point of this subproject is on the
// CONSUMER side: it dials this same server WITHOUT any generated stubs, by
// parsing idl/echo.thrift at runtime and marshaling JSON to the standard
// Thrift wire format (see consumer/main.go). Since the wire format is
// unchanged, the server needs no special "generic" setup — a client using
// generic invocation and a client using generated stubs are indistinguishable
// to it.
