...
type SpringContext interface {
	...
	SetProperties(name string, value string)

	GetPrefixProperties(prefix string) map[string]string
	GetDefaultProperties(name string, defaultValue string) (string, bool)

	AutoWireBeans() error
	WireBean(beanDefinition *SpringBeanDefinition) error
}

type DefaultSpringContext struct {
	beanDefinitionMap map[string]*SpringBeanDefinition
