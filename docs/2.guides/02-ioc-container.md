...
但当应用有几十上百个组件，而且它们之间存在复杂的依赖关系时，我们还能一个个手动组装吗？
能，但是很麻烦。**这时候 IoC 容器就派上用场了** —— 它可以帮助我们自动完成以下所有事情：
...
但是，在一些依赖关系比较简单或者以便捷性为优先的场景中，也可以使用结构体字段注入，
...
如果没有指定 tag 的内容，容器会按照 **Bean 名称的字典序** 对切片中的元素进行排序，
从而确保每次收集到的结果都是确定的，进而保证行为的一致性。
...
type Chain struct {
	...
	Filters []Filter `autow:"auth?,tracing,recovery"`
}
```
...
当然，我们也可以在 Map 中使用可空语法 `name?`，表示当对应的 Bean 不存在时自动跳过。
不过，通配符 `*` 虽然语法上可用，但在 Map 场景下意义不大，因为 Map 本身不保证顺序，
使用 `*` 也无法体现收集顺序上的控制。
...
另外，`${...}` 语法本身支持指定默认值，不过这种用法不常见，这里就不展开介绍了。
...
关于**条件注册**的内容比较多，我们将在本文[**条件注册**](#条件注册)章节进行专门介绍。
