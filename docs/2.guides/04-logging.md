...
- 如何在应用中定义日志标签并输出格式化日志、结构化日志。
- 如何通过标签、级别、Logger、Appender 和 Layout 控制日志行为。
- 如何配置控制台输出、文件输出、滚动文件和异步写入。
- 如何接入链路上下文、统一字段规范，并处理生产环境中的性能和可靠性问题。

## 功能概览

| 能力 | 说明 | 典型场景 |
|------|------|----------|
| 标签路由 | 使用业务语义标签选择 Logger | 订单、支付、RPC、缓存等日志分流 |
| 结构化字段 | 使用强类型 Field 输出键值数据 | 日志检索、统计分析、日志平台采集 |
| 级别范围 | 支持单级别和区间级别过滤 | 开发环境 DEBUG，生产环境 INFO/WARN |
| 多种 Logger | 支持同步、异步和集成式 Logger | 本地调试、高并发服务、滚动文件 |
| 多种 Appender | 支持控制台、文件、滚动文件和丢弃输出 | 容器 stdout、落盘、压测 |
| 多种 Layout | 支持人类可读文本和结构化 JSON | 控制台阅读、ELK/Loki 等日志系统 |
| 上下文提取 | 从 `context.Context` 自动提取 trace 字段 | 分布式链路追踪、请求级排查 |
| 配置刷新 | 原子刷新日志配置 | 动态调整日志级别和输出策略 |

## 核心概念

| 概念 | 作用 |
|------|------|
| Tag | 日志的静态分类，用来决定日志由哪个 Logger 处理。 |
| Field | 日志的结构化内容，用来描述这条日志携带的数据。 |
| Level | 日志级别，用来控制哪些日志会被输出。 |
| Logger | 日志处理入口，负责级别过滤、缓冲策略和分发。 |
| Appender | 日志输出目标，例如控制台、文件或滚动文件。 |
| Layout | 日志编码格式，例如 TextLayout 或 JSONLayout。 |
| Context Hook | 从 `context.Context` 中自动提取 trace_id、span_id 等动态字段。 |
...
使用 TextLayout 时，控制台输出大致如下：

```text
[INFO][2024-01-01T12:00:00.000][main.go:40] _app_startup||msg=应用启动成功，版本: v1.0.0，PID: 12345
[INFO][2024-01-01T12:00:00.001][main.go:43] _biz_order_create||order_id=10001||user=alice||amount=99.99||success=true||tags=[vip,new_user]||msg=订单创建成功
```

在实际项目中，建议按以下步骤接入：

1. 在公共包中统一注册日志标签。
2. 在启动阶段调用 `log.RefreshConfig` 或 `log.RefreshFile` 加载日志配置。
3. 在业务代码中使用 `log.Info`、`log.Warn`、`log.Error` 等 API 输出结构化字段。
4. 在生产环境使用 JSONLayout 和异步写入，便于日志采集和分析。
...
Go-Spring 同时提供格式化日志和结构化日志 API。格式化日志适合简单提示，结构化日志适合生产环境采集、检索和统计。

| API 形式 | 示例 | 适用场景 |
|----------|------|----------|
| 格式化日志 | `log.Infof(ctx, tag, "order %d created", id)` | 简单文本消息、本地调试 |
| 结构化日志 | `log.Info(ctx, tag, log.Int64("order_id", id), log.Msg("created"))` | 生产环境、日志平台检索 |
| 自定义级别 | `log.Record(ctx, level, tag, skip, fields...)` | 审计日志、扩展级别 |
| 惰性求值 | `log.Trace(ctx, tag, func() []log.Field { ... })` | 构造字段成本较高的调试日志 |
...
Logger 负责接收日志事件、执行级别过滤，并将事件分发给一个或多个 Appender。选择 Logger 时，主要关注写入方式、可靠性和对业务 goroutine 的影响。

| Logger | 写入方式 | 适用场景 | 注意事项 |
|--------|----------|----------|----------|
| SyncLogger | 调用 goroutine 同步写入 | 开发调试、低吞吐量服务、不能接受缓冲丢失的场景 | Appender 必须并发安全，写入慢时会阻塞业务 |
| AsyncLogger | 先写入内存缓冲，再由后台 goroutine 写出 | 高并发服务、生产环境常规业务日志 | 需要合理配置缓冲大小和缓冲满策略 |
| ConsoleLogger | 内置控制台输出的快捷 Logger | 本地开发、容器 stdout 输出 | 高并发下控制台输出可能成为瓶颈 |
| FileLogger | 内置单文件输出的快捷 Logger | 简单服务、由外部工具负责日志轮转 | 文件增长需要外部机制控制 |
| RollingFileLogger | 内置滚动文件输出的快捷 Logger | 生产环境落盘、按时间切割和保留 | 需要配置滚动间隔、保留时间和异步策略 |
...
Appender 负责把日志事件写入具体目标。Logger 可以绑定多个 Appender，从而同时写入控制台、文件、告警系统或其他外部系统。

| Appender | 输出目标 | 适用场景 |
|----------|----------|----------|
| DiscardAppender | 丢弃日志 | 压测、测试、临时关闭输出 |
| ConsoleAppender | `os.Stdout` | 本地开发、容器标准输出 |
| FileAppender | 单个文件 | 简单落盘、外部 logrotate 管理 |
| RollingFileAppender | 按时间滚动的文件 | 生产环境日志落盘和自动清理 |
...
Layout 决定日志最终的编码格式。开发环境通常选择可读性更好的 TextLayout，生产环境通常选择便于采集和查询的 JSONLayout。

| Layout | 输出形式 | 推荐场景 |
|--------|----------|----------|
| TextLayout | 单行文本，字段以分隔符拼接 | 本地调试、人工阅读控制台日志 |
| JSONLayout | 单行 JSON 对象 | 日志平台采集、结构化检索、指标统计 |
...
| 配置项 | 说明 | 示例 |
|--------|------|------|
| `logger.<name>.type` | Logger 类型 | `AsyncLogger`、`ConsoleLogger` |
| `logger.<name>.level` | 日志级别或级别范围 | `INFO`、`DEBUG~WARN` |
| `logger.<name>.tag` | Logger 绑定的标签 | `_biz_*`、`_rpc_redis_get` |
| `logger.<name>.bufferSize` | 异步缓冲大小 | `10000` |
| `logger.<name>.onBufferFull` | 缓冲满策略 | `block`、`discard`、`drop-oldest` |
| `logger.<name>.appenderRef.ref` | 引用的 Appender 名称 | `console`、`file` |
| `appender.<name>.type` | Appender 类型 | `ConsoleAppender`、`RollingFileAppender` |
| `appender.<name>.dir` | 日志目录 | `./logs`、`/var/log/app` |
| `appender.<name>.file` | 日志文件名 | `app.log` |
| `appender.<name>.interval` | 滚动间隔 | `1h`、`24h` |
| `appender.<name>.maxAge` | 文件保留时间 | `168h`、`720h` |
| `appender.<name>.layout.type` | Layout 类型 | `TextLayout`、`JSONLayout` |

下面是一份较完整的配置示例，包含环境变量引用、多个 Appender、按标签分流和错误日志独立输出：
...
日志规范最好在项目早期确定。推荐先统一标签和字段命名，再按环境配置输出策略，最后补充敏感信息、性能和退出清理等治理规则。

推荐落地顺序如下：

1. 定义统一的标签包，所有业务代码只引用标签变量。
2. 约定结构化字段命名，保证日志平台中字段含义稳定。
3. 开发环境使用 TextLayout 和控制台输出，生产环境使用 JSONLayout 和滚动文件。
4. 为链路字段设置统一的上下文提取函数。
5. 为高频日志设置级别、采样或限流，避免日志量失控。
6. 在进程退出流程中调用 `log.Destroy()`，确保异步缓冲尽量刷出。
