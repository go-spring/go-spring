...
微服务排障时，一个请求会经过多个服务和组件，产生大量日志。如果每条日志都需要业务代码手动传 `trace_id`、`user_id`、`request_id`，重复且容易遗漏。
...
日志系统的最后一篇会收束配置治理、刷新和生态适配：如何用配置驱动 Logger/Appender/Layout，如何接入标准库 `log` 和 Zap。
