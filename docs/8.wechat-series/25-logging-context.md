# Go-Spring 实战第 25 课：trace_id 总是漏？让日志自动带上请求上下文

前面几篇已经把日志事件从业务调用一路讲到输出管线。现在我们补上一个线上排查时非常关键的细节：上下文字段。

单条日志本身往往不够。一个请求会经过多个服务和组件，产生大量日志。如果每条日志都需要业务代码手动传 `trace_id`、`user_id`、`request_id`，不仅重复，而且很容易遗漏。所以这类字段更适合从上下文统一提取。
...
这些字段通常在请求入口层写入 context，日志系统在输出时统一读取。这样把写入和提取分开，业务代码就不必在每个日志调用点重复拼字段。
...
它适合历史系统或文本格式兼容。如果是新代码，优先使用 `FieldsFromContext`，因为结构化字段保留类型信息。
