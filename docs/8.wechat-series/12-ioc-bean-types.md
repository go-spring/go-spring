...
Bean 创建方式回答的是 Go-Spring 从哪个时刻接手对象。

结构体指针表示对象已经存在，Go-Spring 负责后续注入和生命周期。构造函数表示对象创建也纳入启动期解析，依赖和失败都在创建前后得到处理。函数值表示运行期能力本身要作为 Bean 暴露，而不是被当作构造函数调用。

这个边界清楚以后，容器才能把普通 Go 值放进确定的装配流程里。
