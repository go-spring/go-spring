...
实际选型可以先按这条顺序看：普通业务组件多从构造函数开始；已有对象或简单组件可以用结构体指针；函数本身就是策略或能力时，再把函数指针注册为 Bean。
...
对象可以是临时创建的，也可以是外部已有的实例。注册后，Go-Spring 容器仍会对它执行依赖注入和生命周期回调。对象虽然已经创建，但后续仍然交给容器管理。
...
如果创建过程可能失败，例如配置校验、打开文件、建立连接，可以使用 `(T, error)` 表达失败。容器会在错误时终止启动。
...
参数继续增加时，相关配置聚合成结构体会更清楚，也能避免构造函数里堆满基础类型参数。
