...
Bean 注册看起来只是“把对象放进容器”，但对象从哪里来、什么时候创建、参数怎么注入，都会影响后续行为。

Go-Spring 注册 Bean 时支持多种输入形态。不同形态对应不同创建方式，也影响依赖注入和生命周期管理。
...
这种方式适合简单组件，或需要把既有对象纳入 Go-Spring 管理的集成场景。
...
`BindArg` 还可以附加条件，只有条件满足时才生成对应 Option。
...
构造函数参数较多时，不要把所有配置拆成大量基本类型参数。优先将相关配置聚合为结构体，配置协议会更清楚。
