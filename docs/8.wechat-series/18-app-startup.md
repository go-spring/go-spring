# Go-Spring 实战第 18 课 —— 应用启动链路：`gs.Run()` 如何串起配置、日志、容器和服务

前面几篇把 Go-Spring IoC 的装配边界说清楚了：Bean 在启动期按声明的依赖关系创建，运行期保持普通 Go 对象调用。但一个在线服务真正启动时，问题不止是 Bean 能不能创建出来。配置要先加载，日志要能输出，容器要完成 Bean 创建和注入，启动任务要先于服务执行，`Server` 还要在真正可用后再声明 Ready。

如果这些阶段由业务代码分散调用，启动问题就很难定位。配置什么时候可用、日志什么时候初始化、`Runner` 为什么早于 `Server` 执行、没有 `Server` 时谁来触发 root bean 创建，这些判断都需要一条明确的生命周期链路。
...
启动入口先区分两种场景：当前进程的生命周期由 Go-Spring 接管，还是由外部宿主管理。

独立在线服务通常直接把生命周期交给 Go-Spring。
...
如果 Go-Spring 应用只是嵌入已有进程，调用方还要继续执行自己的生命周期管理，就不能让入口阻塞当前 goroutine。这时使用 `gs.RunAsync()`，让 Go-Spring 完成启动，但把进程等待交还给调用方。
...
代码内置默认值也应该在启动前写入当前 App，作为配置体系里的低优先级兜底值。
...
入口和启动前配置确定以后，Go-Spring 会按固定顺序推进启动阶段。下面这条链路说明的是：配置、日志、容器和服务不是互相独立启动的。
...
Go-Spring 用 `Runner` 表达这类启动任务。它的接口很小，语义也很窄：`Runner` 是启动阶段动作，而不是长期任务容器。
...
应用启动链路把配置、日志、容器、`Runner` 和 `Server` 串成一个有顺序的过程。这个顺序让启动失败可以定位到具体阶段，也让业务代码知道哪些逻辑应该放在启动前配置，哪些应该放进 `Runner`，哪些应该实现为 `Server`。

启动链路清楚以后，`gs.Run()` 就不只是一个入口函数，而是应用从配置输入走到服务 Ready 的编排边界。Go-Spring 在这里把启动期问题前置到确定阶段，运行期代码才能基于已经完成的配置、日志和 Bean 装配结果继续工作。
