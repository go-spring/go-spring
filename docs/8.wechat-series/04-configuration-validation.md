...
下面的注册代码把可复用规则接入表达式环境。函数返回 `bool`，Go-Spring 会把返回值解释成校验是否通过。
...
注册后，字段上的 `expr` 标签就可以像调用内置函数一样调用这些校验函数。这样复杂规则仍然留在启动期校验链路里，而不是分散到业务初始化代码里。
...
校验函数只负责表达规则本身。之后，Go-Spring 负责处理校验失败时的错误传播。
