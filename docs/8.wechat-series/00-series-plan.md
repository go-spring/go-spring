...
当前规划保留 30 个编号，其中第 16 篇内容已合并到第 7 篇和第 15 篇，暂作占位，后续发布时再统一处理编号。系列定位不是零基础入门文，而是面向已经熟悉 Go 工程开发、希望系统理解 go-spring 能力边界和设计取舍的读者。
...
### 16. Profile 条件与装配边界（已合并占位）

本文不再单独成文，内容合并到第 7 篇和第 15 篇。文件暂时保留，后续统一处理编号和发布顺序。
...
- Profile 与配置系统、命名一致性、装配语义：第 7 篇。
- `.OnProfiles()`、普通条件边界、运行期业务分支边界：第 15 篇。
