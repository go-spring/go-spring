...
日志配置用的是扁平化 KV 模型，主要分为三类。命名空间先把 Logger、Appender 和可复用变量分开，后续插件绑定才有稳定入口。
...
我们前面讲过的 Go-Spring 配置模型在这里继续发挥作用，即日志配置本质上也是一组 path，最终绑定到 Logger、Appender 和 Layout 插件上。因为底层仍然是同一套配置模型，日志系统不需要再发明另一套配置规则。

## level 可以表达单级别和范围

`level` 支持单个级别和范围。单级别适合表达“从某个级别开始输出”，范围则适合把不同级别段路由到不同目标。
...
如果一个 Logger 要输出到多个目标，数组配置可以用索引方式稳定表达顺序和局部属性。
...
插件可以实现 `Start()` 和 `Stop()` 参与生命周期管理。资源型 Logger 或 Appender 适合把连接、文件句柄、后台 goroutine 的启动和停止放在这里。

## 转换失败和写入错误要分开处理

插件如果需要自定义配置类型，可以注册转换器。转换失败说明配置本身非法，应该在配置加载或刷新阶段暴露出来。
...
单独使用日志组件时，可以调用 `log.RefreshConfig`。这个入口适合没有接入完整 Go-Spring 应用运行时、但仍希望动态替换日志拓扑的场景。
...
Go-Spring 应用框架内部使用 `log.Refresh` 从配置系统刷新日志配置。这样日志刷新会跟配置系统的刷新入口保持一致。
...
标准库 `log` 通过 `io.Writer` 输出。所以，实现 Writer 即可转发到 Go-Spring。这个适配点适合处理第三方依赖里暂时不能替换的 `log.Print` 调用。
...
Zap 可以通过实现 `zapcore.Core` 适配。核心思路是让 Zap 继续暴露自己的 API，同时把最终写入动作交给 Go-Spring 日志管线。

- `Enabled` 委托给 Go-Spring Logger 判断级别。
- `Write` 将 Zap 事件编码后转发给 Go-Spring Logger。
- `Sync` 交由 Go-Spring 自身生命周期处理。

适配后，项目就可以逐步迁移，即新代码使用 Go-Spring 原生日志 API，旧代码和依赖仍可通过 Zap 输出到同一目标。这样一来，迁移不需要一次性改完所有日志调用点。
