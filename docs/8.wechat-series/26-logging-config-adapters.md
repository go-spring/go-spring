...
配置如何组织，插件如何注入，错误如何上报，配置如何刷新，已有日志入口如何接入，这些问题决定日志系统在真实项目里能否长期维护。所以这一篇收束 Go-Spring 日志系统的配置和生态适配能力。
...
日志配置用的是扁平化 KV 模型，主要分为三类。我们先从命名空间看起。
...
已有项目或第三方库可能按 name 获取 logger。Go-Spring 提供了 `GetLogger`。
...
标准库 `log` 通过 `io.Writer` 输出。所以，实现 Writer 即可转发到 Go-Spring。
...
Zap 可以通过实现 `zapcore.Core` 适配。核心思路如下。
