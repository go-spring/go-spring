...
在前面的例子里，我们都是把目标写在代码里的。但有时候选择可能会跟部署环境或站点配置一起变化，这时候我们可以把注入标签写成 `${...}`，从配置中读取 Bean 名称或名称列表。看一些例子。
...
配置驱动改变的是选择来源，不改变注入目标本身。比如我们在配置中设置了 `storage.provider=oss`，那么表示选择名为 `oss` 的 Bean。如果我们在配置中设置了 `http.filters=auth,tracing,recovery`，那么表示按这个顺序注入过滤器。
