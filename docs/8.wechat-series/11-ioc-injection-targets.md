...
无论使用字段注入还是构造函数注入，Go-Spring 容器最终都要回答一个更具体的问题：应该把哪个值放到这个注入点上呢？
...
我们可以把它们理解成从静态到动态的一组选择。越动态，表达力越强，但排查成本也越高。所以后面看每一种目标时，也要顺手看它的使用边界。

实际使用时可以先抓一条顺序：能按类型唯一注入就不要加名字；需要多实例时再引入名称；只有确实要通过环境切换实现时，再把选择权交给配置。
...
这时候名称就是依赖协议的一部分。我们应该让它稳定、可读，而不是把临时判断藏进业务代码。
...
可空注入适合插件式依赖、可选增强能力或兼容旧系统的过渡场景。核心依赖不建议设为可空，否则错误会被推迟到运行期。也就是说，可空是表达“可选能力”，不是掩盖缺失依赖。
...
这种方式适合通过配置切换实现，例如存储后端、过滤器链、插件列表。这里要注意，配置驱动注入会把依赖关系的一部分移到配置文件里，所以更适合真正需要运行环境切换的点。

## 延迟注入只处理少量循环依赖

延迟注入主要用来处理少量循环依赖，只适合用在结构体字段注入：
...
标记为 `lazy` 的字段会在所有非延迟注入完成后统一处理。在此之前字段保持零值，所以初始化逻辑中不能提前使用它。否则很容易把循环依赖问题变成运行期空值问题。

## 动态注入能力要克制

注入目标越动态，排查成本越高。核心链路优先使用按类型或按名称的显式注入；配置驱动注入适合真正需要运行环境切换的点；可空和延迟注入要保持克制。

注入目标回答的是“依赖给谁”。再往前一步，还要看 Bean 自己从哪里来：结构体指针、构造函数和函数指针会对应不同的创建策略。
