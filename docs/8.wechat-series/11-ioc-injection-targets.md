...
这种方式适合通过配置切换实现，例如存储后端、过滤器链、插件列表。需要注意的是，配置驱动注入会把依赖关系的一部分移到配置文件里，所以更适合真正需要运行环境切换的点。
...
标记为 `lazy` 的字段会在所有非延迟注入完成后统一处理。在此之前字段保持零值，因此初始化逻辑中不能提前使用它。否则很容易把循环依赖问题变成运行期空值问题。
