...
无论使用字段注入还是构造函数注入，Go-Spring 容器最终都要回答一个更具体的问题：哪个值会进入这个注入点？
...
实际使用时可以先抓一条顺序：按类型能唯一注入时直接按类型；出现多实例时引入名称；确实要通过环境切换实现时，再把选择权交给配置。
...
这时候名称就是依赖协议的一部分。名称稳定、可读，后面排查依赖关系也会更直接。
...
可空注入适合插件式依赖、可选增强能力或兼容旧系统的过渡场景。核心依赖如果也设为可空，缺失依赖的错误会被推迟到运行期。这里的重点是表达“可选能力”。
...
这种方式适合通过配置切换实现，例如存储后端、过滤器链、插件列表。它会把依赖关系的一部分移到配置文件里，因此更适合确实需要运行环境切换的点。
