...
如果关闭了内置 HTTP Server，且没有 Runner 或 Server，容器可能没有入口触发 Bean 创建。这时就可以用 `app.Root()` 指定依赖图入口。
...
所有 Server 会在独立 goroutine 中并行启动。每个 Server 应在完成监听绑定或具备服务能力后触发 Ready 信号。Go-Spring 会等待所有 Server Ready 后才认为应用启动完成。

这个机制可以避免健康检查已经开放、流量已经进入，但某个关键 Server 随后启动失败的问题。

## 排查启动问题要先看阶段

启动问题不能只看最后一条错误日志。配置加载、日志初始化、容器启动、Runner 执行、Server 启动，每个阶段都有它自己的失败方式和扩展点。

## 下一篇预告

下一篇会补上生命周期的另一半：应用退出、优雅关闭、长期任务、根 Context 和运行期刷新如何统一管理。
