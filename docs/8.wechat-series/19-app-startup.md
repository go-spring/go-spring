...
前面几篇把 Go-Spring IoC 的装配边界说清楚了：Bean 在启动期按依赖图创建，运行期保持普通 Go 对象调用。但一个在线服务真正启动时，问题不止是 Bean 能不能创建出来。配置要先加载，日志要能输出，容器要完成依赖图，启动任务要先于服务执行，Server 还要在真正可用后再声明 Ready。
...
启动链路清楚以后，`gs.Run()` 就不只是一个入口函数，而是应用从配置输入走到服务 Ready 的编排边界。Go-Spring 在这里把启动期问题前置到确定阶段，运行期代码才能基于已经完成的配置、日志和对象图继续工作。
