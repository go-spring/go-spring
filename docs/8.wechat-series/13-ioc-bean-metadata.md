...
比如有多个同类型的 Bean，主库和从库都是 `*DataSource`，只靠类型就不够了，我们需要给它们命名。比如一个结构体实现了接口，而依赖方希望按接口声明依赖，我们需要明确导出接口。比如对象创建后要检查连接、退出时要释放资源，我们需要把启动和关闭动作交给容器。再比如两个 Bean 没有直接注入关系，但初始化顺序有要求，我们需要显式声明顺序约束。还有些 Bean 只在配置打开或 Profile 命中时启用，我们还需要给它加条件。
...
这段注册语句说明了一个完整的 Bean 使用方式：名称是 `cache`，可以按 `Cache` 接口注入，只有 `redis.enabled=true` 时启用，创建后检查连接，退出时关闭连接。下面就按照这些需求逐个看。
