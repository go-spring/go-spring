...
前面我们已经看过 Go-Spring 的配置怎样表达、怎样绑定、怎样校验。现在可以把视角往前挪一步了，即配置首先得进入系统。
...
格式和来源分离以后，本地文件、环境变量、远程配置中心都可以接入 Go-Spring 的同一套配置模型，后续绑定、校验和合并也就有了统一入口。反过来说，如果这两层混在一起，每增加一种来源或格式，都可能牵动整条配置链路。
