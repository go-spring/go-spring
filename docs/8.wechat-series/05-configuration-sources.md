...
判断一个扩展放在哪里，可以先看变化发生在哪一层：如果输入内容的语法变了，扩展 Reader；如果数据来自新的地方，扩展 Provider。边界清楚后，后续配置能力也能继续复用。
...
Go-Spring 会根据文件后缀自动选择解析器。无论原始格式是什么，最终都会转成统一的 `Properties`。这样后续绑定和校验就不用关心文件格式了：格式可以不同，但进入系统后的表达保持一致。
...
注册后，应用就可以像加载内置格式一样加载 `.ini` 配置文件。新增格式只影响入口解析，后面的配置模型保持不变。
...
多个入口统一以后，还会遇到下一个问题：同一个 key 来自不同来源时谁覆盖谁，以及 Map 和 Slice 这类结构怎样合并。
