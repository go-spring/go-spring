# Go-Spring 实战第 5 课：配置来源扩展：Reader 与 Provider 如何统一文件、环境变量、命令行和配置中心

前面我们已经看过 Go-Spring 的配置怎样表达、怎样绑定、怎样校验。现在可以把视角往前挪一步了，即配置首先得进入系统。

真实应用里的配置不会只来自一个本地文件。它可能来自 YAML、TOML、JSON、环境变量、命令行参数，也可能来自远程配置中心。所以，Go-Spring 没有把所有入口揉在一起，而是把格式解析交给 Reader，把来源读取交给 Provider。
