...
框架会根据文件后缀自动选择解析器。无论原始格式是什么，最终都会转成统一的 `Properties`。这也是前面反复强调配置模型的原因：格式不同，但进入系统后的表达必须一致。
...
不是所有来源都需要框架内置。项目可以按自己的基础设施实现 Provider，只要最终交回统一的配置数据。
...
`optional:` 表示配置不存在时不报错，适合本地覆盖文件、开发者私有配置或非必需的外部配置。
