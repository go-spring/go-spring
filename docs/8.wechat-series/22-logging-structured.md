...
上一篇我们先搭起了日志系统的骨架：标签负责语义路由，Logger 负责调度，Appender、Layout 和 Encoder 负责输出。现在我们回到业务代码真正会写出来的部分：字段。

如果日志只是一段字符串，排查时只能靠全文检索；一旦要统计、聚合、按字段过滤，纯文本就很吃力。

结构化日志把日志内容拆成带类型的字段，让日志平台可以直接索引、聚合和过滤。字段一旦稳定下来，日志就不只是“给人看的文本”，也是系统观测的一部分。
...
字段函数直接携带类型信息，编码时不需要重新推断。这也是它比 `map[string]any` 更适合高频日志路径的原因。
...
换句话说，我们可以把 `msg` 当成一句话摘要，但不要把可检索的业务信息都塞进这句话里。
...
嵌套对象适合表达一个字段下的结构化子对象，尤其是 JSON 输出。
...
这种方式适合已有动态字段集合的场景。对于稳定字段，仍建议直接使用强类型字段函数。
...
字段模型确定之后，日志事件还需要被调度和落地。下一步进入 Logger 体系，讨论同步、异步、控制台、文件、滚动文件以及自定义 Logger 的适用边界。
