# Go-Spring 实战第 22 课 —— 结构化日志：把业务事件写成可检索字段
...
Go-Spring 的 Field 系统用强类型字段表达日志内容。字段名、字段类型和字段值在写日志时就确定下来，后续 Layout 和 Encoder 才能稳定输出给日志平台。因此这一篇只围绕一个问题展开：业务事件里的信息，哪些应该成为字段，哪些只适合留在 `msg` 里。

## 基础字段要在调用点声明类型

稳定字段应该优先使用强类型字段函数。这样字段名和值类型在调用点就已经确定，编码时不需要再从 `any` 里推断。

下面这条登录日志证明的是字段拆分方式：可检索信息分别写成字段，最后用 `Msg` 留一句人类可读摘要。
...
有些字段不是没有，而是当前值为空。下面的例子证明 Go-Spring 可以显式表达“字段存在但值为 null”。
...
`Msg` 和 `Msgf` 是特殊字段，key 都是 `msg`。下面的例子只证明它们适合保存一句人类可读摘要，而不是承载所有业务信息。
...
当一个字段天然是列表时，直接使用数组字段函数。下面的例子证明 Go-Spring 会保留列表结构，而不是把列表拼成字符串。
...
当多个字段共同描述一个业务对象时，用 `Object` 把它们收进同一个结构下。下面的订单对象证明对象字段可以继续嵌套。
...
有些日志字段来自外部系统或动态属性集合，调用点并不知道完整字段列表。下面的例子证明 `FieldsFromMap` 可以把这类 `map[string]any` 展开成多个字段。
...
`Any` 会根据值自动选择编码方式。下面的例子证明它可以承接调用点暂时无法明确类型的值。
