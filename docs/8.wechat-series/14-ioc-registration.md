...
这些入口可以先按组织边界来看：单个对象、动态模块、多实例配置、配置类组织，还是当前应用实例专属。边界清楚了，入口也就容易选。
...
`gs.Provide()` 发生在应用启动前。如果启动之后再追加全局 Bean，就会破坏容器启动期解析模型。
...
Module 更适合 Starter 和组件包：它可以读取配置、判断条件，然后批量注册 Bean。当注册动作本身依赖配置或环境时，这段逻辑放进 Module 会比一串 `Provide` 调用更自然。
...
这种方式常用在测试里，或者需要在当前启动实例中补充 Bean 的场景。它不会污染全局注册表，所以更适合临时替换和局部组装，解决的是“当前这次启动”的注册问题。
