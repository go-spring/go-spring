...
在 Go-Spring 项目里，当只有几个对象时，注册 Bean 并不难；真正容易变复杂的是组件变多之后，注册逻辑放在哪里、怎样按配置批量展开、怎样封装成可复用模块。
...
这些入口可以先按组织边界来看，即单个对象、动态模块、多实例配置、配置类组织，还是当前应用实例专属。边界清楚了，入口也就容易选。

## gs.Provide 处理普通业务 Bean
...
它会把 Bean 定义记录到 Go-Spring 全局注册表，在应用启动时统一合并。绝大多数业务组件都会走这条注册路径。

`gs.Provide()` 发生在应用启动前。因为 Go-Spring 会在启动期一次性解析依赖图，如果启动之后再追加全局 Bean，就会破坏这个模型。
...
Module 更适合 Starter 和组件包——它可以读取配置、判断条件，然后批量注册 Bean。也就是说，当注册动作本身依赖配置或环境时，这段逻辑放进 Module 会比一串 `Provide` 调用更自然。
...
`Provide`、`Module`、`Group`、`Configuration` 和 `app.Provide` 解决的是“如何把 Bean 放进容器”。不同入口对应不同组织方式，即单个对象、动态模块、批量实例、配置类和测试应用。

Bean 放进容器只是第一步。真正启动时，容器还会根据配置、环境和已有依赖判断哪些定义生效，这就是条件注册要处理的问题。
