...
所以这一篇我们不只看“怎么注册”，还要看不同注册入口背后的组织边界。Go-Spring 提供多种注册入口，它们不是重复能力，而是分别面向不同场景：
...
需要注意的是，`gs.Provide()` 必须在应用启动前调用。启动之后再追加全局 Bean，会破坏容器启动期解析模型。
...
Module 适合 Starter 和组件包：它可以读取配置、判断条件，然后批量注册 Bean。也就是说，当注册动作本身依赖配置或环境时，我们就不应该硬塞进一串 `Provide` 调用里。
...
Configuration 适合一组强相关 Bean 的集中组织。它让相关创建逻辑放在同一个类型里，同时仍然把返回值作为独立 Bean 管理。
...
这种方式常用于测试，或者需要在当前启动实例中补充 Bean 的场景。它不会污染全局注册表，因此更适合临时替换和局部组装。
