# Go-Spring 实战第 30 课 —— 能力地图：用应用生命周期串起配置、IoC、运行时和验证
...
Go-Spring 不是并排摆放的一组工具函数。它的能力地图围绕应用生命周期展开：配置提供输入，IoC 组织对象，运行时协调阶段，日志和 HTTP 接入运行态，Starter 复用组件规则，测试持续验证结果。理解这条链路，比记住单个 API 更重要。
