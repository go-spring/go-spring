...
多环境配置最容易写成复制粘贴。开发、测试、生产环境往往只在少量配置上不同。合理的做法应该是让基础配置复用，让环境差异独立覆盖，而不是为每个环境复制一整份配置，然后在几份文件里反复同步相同内容。
...
这里的顺序是有意义的，后面会影响多个 Profile 之间的覆盖关系。
...
这种方式鼓励我们只在 Profile 文件中写差异配置。基础配置越稳定，Profile 文件就越容易阅读。
