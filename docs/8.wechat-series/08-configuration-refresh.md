...
到这里，Go-Spring 配置系统的主线已经基本走完了。前面几篇里，我们先用 `Properties` 和 `path` 建立统一配置模型，再把配置绑定到 Go 类型；随后处理了 `Duration`、`Time`、`Slice`、`Map` 等复杂类型，使用 `expr` 标签拦截非法配置；再往后，Reader、Provider、环境变量和命令行参数把不同来源接入进来，优先级、合并语义和 Profile 则回答了多来源、多环境配置如何得到最终值。

这一篇作为配置系列的最后一篇，还剩两个比较重要的问题需要收束。第一个问题发生在启动期：同一个地址、目录、端口或环境变量可能要被多个配置项复用，如果每个地方都手写一遍，后续修改很容易遗漏。第二个问题发生在运行期：开关、阈值、超时时间这类轻量参数，有时希望在不重启进程的情况下读取新值。
