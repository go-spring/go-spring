# Go-Spring 实战第 2 课：配置绑定：把 Properties 安全映射到 Go 结构体

上一篇我们把 Go-Spring 中不同来源、不同格式的配置统一到了 `Properties` 和 path 空间里。模型统一以后，下一个问题就很现实了：这些配置怎样进入 Go 代码。

配置只有绑定到结构体、函数参数或模块注册逻辑里，才算真正开始影响应用行为。Go-Spring 提供了两种主要绑定方式：

- 结构体标签绑定，适合绝大多数业务代码。
- 手动 `Bind` 函数绑定，适合模块化注册、批量创建 Bean 等更底层的场景。

这两种方式共享同一套配置 path 和类型转换机制，只是使用位置不同。接下来先看入口，再沿着同一条绑定链路看复杂类型、校验和动态注册。

## 用 value 标签把配置声明在字段旁边
...
手动 `Bind` 没有引入新的配置模型，它只是把同一套绑定能力暴露给更底层的注册代码。
