# Go-Spring 实战第 9 课：Go 项目写大以后，为什么会需要 IoC 容器

配置系统解决的是“应用怎样从外部获得参数”。从这一篇开始，我们转到另一个核心问题：应用里的对象怎样被创建、组合和管理。

讲 IoC 容器之前，先把一个误会放到桌面上：Go-Spring 做 IoC，不是为了把 Java Spring 的那套东西搬进 Go。Go 语言鼓励显式依赖和简单组合，这一点没有变。
...
具体实现由外部传入，组件边界更清楚，也更容易测试。所以这里并没有什么神秘机制，本质就是把“创建依赖”这件事从业务对象里拿出来。
...
但当应用有几十上百个组件时，手动维护依赖顺序会迅速变得繁琐。因为我们不只是要把对象 new 出来，还要处理条件启用、生命周期回调、测试替换和退出清理。
...
我们在 `init()` 中注册 Bean，构造函数声明依赖，最后用 `gs.Run()` 启动应用。接着，容器会完成对象创建、依赖注入和 HTTP Server 启动。

## Bean 是什么

Bean 是由容器管理的组件。它可以是服务、控制器、路由器、客户端、配置类，也可以是实现了 `Runner` 或 `Server` 的运行单元。

在 Go 中，包是代码组织单位，但包的粒度偏粗，一个包里可能包含多个运行期对象。Bean 则是运行期组织单位，用于精确描述依赖关系和生命周期。这样我们讨论对象装配时，就不用把问题混在包边界里。
...
有了这个定位，我们下一步看两种最基础的注入方式：结构体字段注入和构造函数参数注入。
