...
这里，我不想讲 Go-Spring 的能力如何如何强大，我想讲一下——直觉，也就是发自心底的原始的认知：我们想要组织对象之间的依赖关系应该怎么办？首先，我们需要把对象自己的信息暴露出去，也就是注册到 IoC 容器里面。然后呢？其实没有然后了，已经够了。每个对象在注册自己的时候已经声明了它需要什么样的依赖。只要依赖图上的所有依赖都注册了自己，IoC 容器就可以根据这些声明，自动地完成对象之间的装配。妙哉！
