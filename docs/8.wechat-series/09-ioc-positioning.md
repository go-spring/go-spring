...
上一篇我们结束了 Go-Spring 的配置体系，大家系统性地了解了 Go-Spring 怎样解决 Go 应用的配置绑定、校验、加载等问题。
...
一个对象最核心的职责，应该是使用它的依赖完成自己的业务逻辑，而不是顺手把下游依赖也创建出来。创建者和使用者一旦混在一起，代码在早期可能不会暴露出明显问题，但是当依赖变得更多、实现需要替换、测试需要隔离时，就会越来越难维护。

**我们更推荐把创建者和使用者分开**。一个对象只需要通过构造函数声明自己需要什么依赖就可以了，至于这个依赖从哪里来、使用真实实现还是 mock 实现，则交给外部来决定。
...
在这个版本里，`UserController` 不再关心 `UserService` 如何创建，它只通过入参表达了自己需要一个 `UserService` 的需求。换句话说，我们可以将对象需要什么依赖直接写在构造函数签名上，而不是藏在内部实现里。
...
在此之上，更进一步，IoC 容器还可以统一对象的生命周期管理。这一点在后面的文章中会逐渐展开。
