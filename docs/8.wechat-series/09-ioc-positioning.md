...
Go-Spring 配置系统解决的是“应用怎样从外部获得参数”。从这一篇开始，我们转到另一个核心问题，即应用里的对象怎样被创建、组合和管理。

讲 IoC 容器之前，先把一个容易误会的点放到桌面上——Go-Spring 做 IoC，不是为了把 Java Spring 的那套东西搬进 Go。Go 语言鼓励显式依赖和简单组合，这一点没有变。

那 Go-Spring 为什么还需要容器呢？关键在于，大型 Go 应用里会遇到另一类问题，即对象越来越多、依赖关系越来越复杂、启动和销毁顺序需要统一管理、测试环境需要替换依赖、不同环境还要启用不同实现。
...
但当应用有几十上百个组件时，手动维护依赖顺序会迅速变得繁琐。原因也很直接——我们不只是要把对象 new 出来，还要处理条件启用、生命周期回调、测试替换和退出清理，这些事情很快就会叠在一起。
...
下面的例子把注册、构造函数依赖和 HTTP 路由放在一起。阅读时重点看三件事，即 `UserController` 不自己创建 `UserService`，构造函数声明依赖，最后由 `gs.Provide` 把这些对象交给容器。
...
在 Go 中，包是代码组织单位，但包的粒度偏粗，一个包里可能包含多个运行期对象。Bean 则是运行期组织单位，用来精确描述依赖关系和生命周期。换句话说，我们讨论对象装配时，就不用把问题混在包边界里。

这也是 Go-Spring 引入 Bean 的原因，即它补充的是 Go 语言运行期对象管理能力，而不是替代包和函数。
