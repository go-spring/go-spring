...
上一篇我们收束了配置系统。配置来源、Profile、imports、变量引用和动态刷新，最终都回到同一套 `Properties` 语义里。到这一步，外部参数已经能以确定的规则进入应用，并在启动期或运行期变成 Go 值。

但参数进入应用之后，紧接着就会落到另一个工程问题上：对象由谁创建，依赖由谁连接，初始化和退出顺序又由谁保证。

小型 Go 服务里，手写 `NewXxx()` 再逐层传参通常很直接。随着组件数量增加，问题会从“能不能 new 出对象”变成“能不能稳定维护一张对象关系图”。同一个服务里可能同时有路由、仓储、客户端、后台任务、可选插件和测试替身，它们还会受到配置、Profile 和部署环境影响。

Go-Spring 做 IoC，不是把 Java Spring 的使用方式搬进 Go。Go 语言鼓励显式依赖和简单组合，这个前提没有变。Go-Spring 的 IoC 容器补上的，是大型应用里对象装配、条件启用和生命周期管理的统一启动入口。
...
这个例子证明的不是手工装配不能用，而是对象数量增加后，代码需要维护的已经不只是创建顺序。真实服务还会继续叠加条件启用、初始化回调、退出清理、测试替换和多环境实现选择。

如果这些规则都散落在 `main()`、各个包的初始化函数和测试代码里，应用很快会出现两类问题：一类是对象关系变得难以看清，另一类是启动失败、资源释放和环境差异没有统一语义。

Go-Spring 的 IoC 容器接管的是这些启动期工作。

| 工作 | Go-Spring 语义 |
|------|----------------|
| 注册对象来源 | 收集应用声明的 Bean 定义 |
| 分析依赖关系 | 根据构造参数、字段标签和元信息建立对象图 |
| 创建和注入 | 按依赖顺序创建对象，并把依赖写入目标位置 |
| 生命周期管理 | 在统一顺序里执行初始化、运行和销毁动作 |
| 条件选择 | 根据配置和环境决定哪些 Bean 参与本次装配 |

所以，Go-Spring 的重点不是让业务代码看起来更“框架化”。它要统一的是对象装配、生命周期和条件启用这些原本分散在各处的规则。
