...
在自动装配的想法出现之前，我们都是手写装配的，那么每个 bean 是否存在，在注册的时候就已经确定了。不过这也限制了代码的表达能力。在出现自动装配之后，我们提供了复用性更强的代码和包，但是通用意味着对场景的限制不是死的。所以用户可能引入不使用的 bean，小问题是创建了用户不需要的 bean，严重的时服务根本起不来，因为需要的配置或者环境根本不存在。

这时候，我们就需要条件来控制 bean 是否启用。
