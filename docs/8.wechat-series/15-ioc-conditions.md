...
另外，Go-Spring 在判断 Bean 是否存在时，会跳过已经条件判断失败被删除的 Bean。换句话说，它不会再被当成一个“存在的 Bean”参与匹配。

## 自定义条件

Go-Spring 内置的条件已经覆盖了绝大多数场景。但是如果规则已经超出了配置值和 Bean 存在性，或者更复杂的逻辑判断，那么可以使用 `OnFunc`。
...
在上面的代码中，我们通过 `OnFunc` 创建了一个条件：只有 `audit.mode` 存在并且值不是 `off` 时，`NewAuditSink` 才会启用。我们可以在 `OnFunc` 里写更清晰的判断逻辑。
...
虽然组合条件能表达复杂关系，但是条件越复杂，注册语句也越难读。一方面，我们可以将组合条件提取成变量，这样可读性更高。另一方面，我们应当谨慎使用条件组合。当出现复杂的条件组合时，我们应该考虑是否应该调整配置边界或者模块设计。

## 条件结果缓存

有时候，多个 Bean 的装配条件有相同部分，而且这部分写法较长或执行成本较高，此时我们可以使用 `OnOnce` 来共享同一个条件实例。`OnOnce` 会缓存第一次匹配结果，后续再次判断同一个条件实例时，直接返回缓存结果。
...
不过，需要提醒的是，`OnOnce` 不应该是默认选择。简单条件直接重复写通常更清楚；只有它确实减少重复、提升可读性，或者条件计算本身有明显成本时，才值得使用。
