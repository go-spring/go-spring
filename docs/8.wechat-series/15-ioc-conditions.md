...
手写装配时，组件什么时候创建，通常由应用代码自己决定：配置了 Redis 才连，打开监控开关才注册监控，要不要使用默认客户端也写在本项目的初始化逻辑里。

自动装配出现以后，注册逻辑开始移动到 Starter 里。Starter 面对的是很多不同应用：有的有配置，有的没有；有的需要默认实现，有的已经提供了自己的实现；有些增强组件也只有在基础组件存在时才有意义。导入 Starter 不能等于注册全部候选 Bean。

Condition 就是在容器解析阶段筛选候选 Bean 的机制。它根据配置、已有 Bean 和上下文决定谁参与本次装配，让 Starter 能提供默认能力，也允许应用覆盖和裁剪。
