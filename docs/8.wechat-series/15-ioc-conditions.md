...
在自动装配出现之前，装配逻辑大多写在应用代码里。每个 Bean 是否注册，通常在本项目的初始化阶段就已经确定：配置了 Redis 才注册 Redis 客户端，打开监控开关才注册监控组件。

自动装配把注册逻辑抽到 Starter 里，复用性更强，但它面对的也不再是单一场景。应用可能引入了某个 Starter，却没有提供对应配置；也可能已经有自己的实现，不需要 Starter 的默认 Bean。如果这些候选 Bean 被无条件放进容器，轻则创建多余组件，重则因为缺少配置、依赖或运行环境导致启动失败。

Condition 就是为这类候选 Bean 加上启用条件。它在容器解析阶段根据配置、已有 Bean 和上下文决定谁参与本次装配，让 Starter 既能提供默认能力，也给应用留下覆盖和裁剪空间。
