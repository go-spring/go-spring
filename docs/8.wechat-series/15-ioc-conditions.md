...
某些 Bean 只在配置存在时启用，某些实现只在缺少默认实现时启用，某些模块只在特定环境中启用。如果这些分支全部写进业务代码，模块边界会很快变得混乱。
...
这样做的好处是，业务代码不需要在运行期反复判断“这个组件该不该存在”。该不该装配，启动时就已经决定。
...
表达力足够强，但工程上应避免让条件变成难以推理的布尔表达式。我们希望条件读起来像装配规则，而不是一段隐藏的业务流程。
