# Go-Spring 实战第 15 课 —— 条件注册：用配置、Bean 存在性和环境差异裁剪对象图

上一课把 Bean 注册入口按组织边界拆开以后，还有一个问题会马上出现：Bean 定义进入 Go-Spring 容器以后，是否就一定会参与本次启动。

真实项目里的组件经常带有启用条件。Redis 客户端只在配置打开时注册，Starter 默认实现只在应用没有自定义实现时生效，某些日志或监控组件只在特定环境中启用。如果这些分支都写进业务代码，模块边界会很快混在一起。
...
真实模块经常需要多个条件同时成立。例如某个服务既要打开配置开关，又要依赖另一个 Bean 已经存在。Go-Spring 提供了 `And`、`Or`、`Not`、`None` 来组合条件。
