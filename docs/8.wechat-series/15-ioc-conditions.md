...
需要注意的是，组合条件虽然很强大，但是条件越复杂，注册语句也就越难读。因此，在我们遇到复杂的组合条件时，需要特别注意如何处理代码的可读性。

首先，我们需要思考这么复杂的条件组合是否必要，是不是配置边界或者模块设计的有问题，才导致这么复杂的条件组合。

然后，我们可以将组合条件提取成变量，这样不仅可以复用，同时也会让注册语句更清楚。
