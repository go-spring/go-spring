...
Go-Spring 内置的条件已经覆盖了绝大多数场景。不过，如果判断规则已经超出了配置值和 Bean 存在性，比如需要读取多个上下文信息，或者需要更细的业务判断，那么可以使用 `OnFunc`。
...
在上面的代码中，我们通过 `OnFunc` 创建了一个条件：只有 `audit.mode` 存在并且值不是 `off` 时，`NewAuditSink` 才会启用。相比于把复杂逻辑塞到字符串表达式里，`OnFunc` 显然更适合承载这种稍微复杂的判断。

## 条件组合

有些场景下，可能单个条件不够，需要把多个条件合在一起进行判断。比如我们希望某个 Bean 只有在配置开关打开时才会启用，同时又要求必须存在某个 Bean 。这种逻辑在 Starter 里很常见。
...
> 对于上面的代码，我们当然也可以使用 `OnMissingBean` 来实现。这里仅仅举个例子。
...
> 上面的代码表示只有当 `reporter.file` 和 `reporter.remote` 都没有打开时，`NewDefaultReporter` 才会启用。`None` 使用比较少见。
...
> 上面的代码仅仅是为了展示用法，实际场景中还是应该根据具体需求来设计条件组合。

## 条件注册

需要注意的是，条件虽然很强大，但是条件越复杂，注册语句也就越难读，对 Bean 条件的理解也越困难。因此，我们在遇到复杂的（组合）条件时，需要特别注意以下两点：

- 我们需要思考如此复杂的（组合）条件是否真的必要，是不是配置边界或者模块设计出了问题，才导致这么复杂的条件组合。
- 我们可以将（组合）条件提取成变量，然后在多个 Bean 注册时使用，这样不仅可以代码复用，同时也会让注册逻辑更清楚。
