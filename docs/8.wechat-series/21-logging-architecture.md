# Go-Spring 实战第 21 课：日志不是 println，从打印文本到可治理链路
...
如果这些能力没有架构约束，日志代码会很快变成一组难以治理的全局开关。所以 Go-Spring 日志系统采用基于标签路由的分层架构，把这些职责拆成可组合组件。
...
格式化日志适合简单提示；结构化字段适合业务事件和线上检索。后面会单独展开结构化字段，这里先看整体链路。
...
标签回答的是“这条日志是什么性质”，而不是“这条日志来自哪个包”。这样比传统按包名路由更贴近业务语义。
