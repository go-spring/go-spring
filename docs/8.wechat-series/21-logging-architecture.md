...
Go-Spring 的日志系统很容易被低估为几个 `Infof` 加一个文件输出。但真实应用里的日志要解决更多问题，即日志分类、级别过滤、结构化字段、上下文提取、异步写入、多目标输出、格式化和配置刷新。
...
这些层次分开以后，我们就可以独立调整路由、过滤、落地目标和输出格式，而不用把所有逻辑塞进一个 Logger 实现里。因为每一层只回答一个问题，后续扩展时影响面也会更小。

## 标签按业务语义来路由

标签回答的其实是“这条日志是什么性质”，而不是“这条日志来自哪个包”。所以它比传统按包名路由更贴近业务语义。
...
标签路由用的是精确优先、最长优先。例如 `_biz_order_create` 会依次匹配下面这些规则。
...
线上业务日志通常优先结构化，因为这样更便于日志平台索引、聚合和检索。
...
骨架立住以后，业务代码最先接触到的就是字段模型，即基础类型、指针、消息字段、数组、对象、Map 展开和 `Any` 分别适合不同日志内容。
