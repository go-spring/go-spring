# Go-Spring 实战第 21 课 —— 日志架构：标签路由与输出管线
...
下面的例子证明一件事：调用点只需要说明日志属于哪类事件，以及这条事件携带哪些字段，不需要在业务代码里选择输出目标。
...
这段代码证明的是低级别日志的计算边界。只有当这条 Debug 日志通过级别判断后，函数才会执行。这样低级别日志可以保留在代码里，但不会在关闭状态下持续消耗业务路径。
...
这套架构的价值不是让概念变多，而是让每个概念只承担一类责任。业务代码稳定地面对标签、级别和字段；Go-Spring 则把路由、过滤、调度、格式和编码放到独立层次中处理。后面无论调整输出格式、接入滚动文件，还是补充上下文字段，都不需要把这些差异重新挤回业务调用点。
