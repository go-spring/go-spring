...
日志系统不只是 `Infof` 和文件输出。真实应用需要解决日志分类、级别过滤、结构化字段、上下文提取、异步写入、多目标输出、格式化和配置刷新。

Go-Spring 日志系统采用基于标签路由的分层架构，把这些职责拆成可组合组件。
...
本篇建立日志系统整体架构。下一篇会深入结构化日志字段模型，讨论基础类型、指针、消息字段、数组、对象、Map 展开和 `Any`。
