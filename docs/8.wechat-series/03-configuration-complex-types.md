...
有了结构体绑定以后，我们已经可以把配置放进 Go 类型里。但如果只把配置当字符串读，业务代码很快就会重新长出一堆手动解析逻辑：超时时间要 `time.ParseDuration`，时间点要单独解析，列表和连接字典还要自己拆分。

真实配置很少只有字符串和整数。超时时间、时间点、枚举、列表、对象数组、连接字典都很常见。Go-Spring 的配置绑定围绕 Go 类型系统展开，目标是让配置结构自然映射到业务结构体，而不是把解析细节散落到业务代码里。
...
如果目标类型是结构体，且没有对应转换器，框架会递归绑定结构体字段。这也是嵌套配置能够自然落到嵌套结构体上的原因。
...
转换器是全局注册的，应在程序启动前完成注册，通常放在 `init()` 中。这样容器启动时，绑定阶段就已经知道如何把字符串转换成目标类型。
...
这种方式非常适合多数据源、多 Redis、多 HTTP 客户端这类按名称管理的实例配置。我们把实例名称放在配置路径里，把实例结构交给 Go 类型系统承接。

## 复杂类型不是额外负担

复杂类型绑定的价值，是让业务结构自然承接配置结构。超时时间、连接列表、命名实例和嵌套对象都可以留在类型系统里，而不是退回字符串解析和手工拼装。

类型能接住配置以后，下一步要补上的就是合法性：绑定成功不代表配置一定可用，我们还需要在启动阶段把错误挡住。
