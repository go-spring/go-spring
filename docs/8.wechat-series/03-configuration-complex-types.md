...
真实的配置中很少只会有字符串和整数，超时时间、时长、枚举、列表、对象数组、字典等等，都是很常见的。如果 Go-Spring 只支持整数和字符串这些简单类型，那么我们就需要手动处理这些复杂类型。这必然是不行的。Go-Spring 需要支持更复杂的类型，这样才能让配置结构自然映射到业务结构体，而不是把解析细节散落到业务代码里。
