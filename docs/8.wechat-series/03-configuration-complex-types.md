# Go-Spring 实战第 3 课：告别手写解析，让复杂配置直接变成 Go 类型

有了结构体绑定以后，我们已经可以把配置放进 Go 类型里。但如果只把配置当字符串读，业务代码很快就会重新长出一堆手动解析逻辑：超时时间要 `time.ParseDuration`，时间点要单独解析，列表和连接字典还要自己拆分。

真实配置很少只有字符串和整数。超时时间、时间点、枚举、列表、对象数组、连接字典都很常见。换句话说，如果绑定层只能处理简单值，业务代码就会被迫接手大量解析工作。Go-Spring 的配置绑定围绕 Go 类型系统展开，目标是让配置结构自然映射到业务结构体，而不是把解析细节散落到业务代码里。
...
转换器是全局注册的，因此应在程序启动前完成注册，通常放在 `init()` 中。这样容器启动时，绑定阶段就已经知道如何把字符串转换成目标类型。
