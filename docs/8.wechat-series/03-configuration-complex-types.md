# Go-Spring 实战第 3 课：复杂配置绑定：Duration、Time、Slice、Map 如何保持类型语义
...
真实配置很少只会有字符串和整数。超时时间、时间点、枚举、列表、对象数组、连接字典都很常见。如果绑定层只能处理简单值，业务代码很快就会接手大量解析工作。所以 Go-Spring 的配置绑定围绕 Go 类型系统展开，目标是让配置结构自然映射到业务结构体，而不是把解析细节散落到业务代码里。

## 基础类型转换先守住最小可用范围

复杂类型绑定要先建立在稳定的基础类型转换之上。Go-Spring 开箱支持常见基础类型。

- `bool` 支持 `true`/`false`、`1`/`0`、`t`/`f` 等写法。
- 整数类型支持 `int`、`int8`、`int16`、`int32`、`int64` 以及对应无符号类型。
- 浮点类型支持 `float32`、`float64`，包括科学计数法。
- `string` 按原始字符串绑定。
...
基础类型之外，Go-Spring 内置了常用类型转换器，最典型的是下面两类。
...
下面的转换器把字符串配置映射成业务枚举。这里要看的不是 `switch` 本身，而是函数签名，因为 Go-Spring 会根据返回值类型知道这个转换器负责 `Status`。
...
转换器是全局注册的，通常放在 `init()` 中，在程序启动前完成注册。这样做的原因也很直接——Go-Spring 容器启动时，绑定阶段就已经知道如何把字符串转换成目标类型。

## Slice 和 Array 仍然复用 path 下标语义
