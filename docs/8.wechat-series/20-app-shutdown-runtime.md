...
服务能启动只是第一步。线上服务更难处理的是退出时机：容器平台发送停止信号后，HTTP 请求可能还没处理完，后台任务还在循环，日志缓冲区里还有数据，配置刷新也可能正在发生。
...
下面这段代码要证明的是：独立服务不需要自己再写一套 signal 监听。
...
退出不是简单取消一个 context。Go-Spring 需要先让对外服务停止接收或处理新工作，再释放容器里的对象。下面这条链路要证明的是：关闭顺序和启动顺序一样需要编排。
...
关闭问题经常来自启动阶段的职责放错位置。`Runner` 属于启动阶段，而不是运行期任务容器。下面这个例子要证明的是：Runner 完成一次性准备动作后必须返回。
...
真正需要持续运行的能力应该实现 `Server`。下面这个简化 HTTP Server 要证明的是：Server 的 `Run` 和 `Stop` 对应同一项长期服务的启动和收束。
...
Go-Spring 在容器里提供 `ContextProvider`。下面这个例子要证明的是：业务对象可以从应用级 root context 派生自己的上下文。
...
启动配置默认是稳定值。只有需要在运行期读取新值的字段，才用 `gs.Dync[T]` 包装。下面这个例子要证明的是：普通字段和动态字段在语义上是两类配置。
...
这条边界让启动之外的行为也可以治理。服务启动、退出、长期任务和动态配置刷新都进入同一个应用生命周期后，后续运行期发生的请求、任务、依赖调用和错误，才有条件继续通过日志系统进入可观察的链路。
