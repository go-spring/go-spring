...
`gs.RunAsync()` 不会自动接管调用方的进程生命周期。所以使用它时，调用方应该在自己的退出流程中调用 `stop()`。
...
下面这个 Runner 把数据库迁移放在服务启动前执行，适合“必须完成后才能对外提供服务”的准备动作：
...
Runner 在 Server 启动前执行，所以适合处理服务可用前必须完成的准备工作。我们不应该把长期循环任务塞进 Runner，否则启动流程会被阻塞。
...
下面这个简化 HTTP Server 展示了关键顺序：先监听端口，监听成功后触发 Ready，停止时调用 `Shutdown`：
...
Ready 信号应该在监听成功后触发，而不是在真正具备服务能力前提前触发。这样 Go-Spring 等待 Ready 才有实际意义。

## 业务 goroutine 应从 root context 派生
