# Go-Spring 实战第 20 课：服务能启动只是开始，优雅退出才见工程功底

一个服务真正可靠，不只看它能不能启动，还要看它如何退出、如何管理运行中的任务。

在线服务需要处理退出信号、优雅关闭、长期运行任务、根 Context 和动态配置刷新。如果这些能力散落在各个业务模块里，生命周期边界会很难统一。
...
Ready 信号应在监听成功后触发，而不是在真正具备服务能力前提前触发。这样框架等待 Ready 才有实际意义。

## 注入根 Context

业务代码应优先从应用 root context 派生上下文。这样应用关闭时，root context 会被取消，所有监听 `ctx.Done()` 的逻辑都能收到通知。
...
启动、退出、长期任务、根 Context 和动态配置刷新合在一起，才是一个在线应用完整的运行时。Go-Spring 把这些能力收进统一生命周期，业务模块就不用各自发明一套关闭和刷新协议。
