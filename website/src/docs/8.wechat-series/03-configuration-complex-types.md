...
上一篇咱们讲了配置绑定。在配置从 `Properties` 进入业务结构体以后，应用代码拿到的就不再是一组字符串，而是已经转换过的 Go 值。
...
下面这个示例展示了自定义转换器的使用。这里使用一个转换器把字符串转换成枚举类型 `Status`。首先，咱们需要注册这个转换器。
...
复杂类型绑定让业务结构可以自然地承接配置结构，自定义转换器则把业务类型接入同一套绑定体系，进一步扩展了 Go-Spring 对配置语义的表达能力。
