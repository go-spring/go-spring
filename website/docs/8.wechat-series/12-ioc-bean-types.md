...
前面两篇文章，讲了依赖注入的写法和注入目标，本篇咱们来详细讲讲 Bean 的注册类型。

所谓注册类型，指的是注册 Bean 时，传给 `gs.Provide()` 的第一个参数是什么类型。这个参数可以是一个已经创建好的结构体指针，可以是一个用来创建对象的构造函数，也可以是一个函数本身。
...
在上面的注册语句里，`new(MyService)` 是传给 `gs.Provide()` 的参数，它是一个结构体指针。这种情况下，Go-Spring 不会再创建 `MyService` 对象，因为对象已经存在了，直接拿来复用就好了。容器要做的是接管这个对象，继续完成字段注入、生命周期回调、条件判断、名称和接口导出等容器语义。
...
像配置校验、打开文件、建立连接这类动作都适合通过 `(T, error)` 来表达失败。Go-Spring 在收到非 nil 错误后会立即终止启动，而不是让半初始化的对象进入运行期，这样可以避免带来更复杂的错误。
...
如果构造函数的参数比较少，我们可能会按顺序逐个进行绑定。但如果参数较多，而且有些参数可以由容器自动推断，这时候就可以使用 `IndexArg` 绑定参数的下标，来跳过不需要显式绑定的参数。
...
Go-Spring 不提供 prototype 作用域，容器中的所有 Bean 都是单例的。

这种设计适合大多数服务端组件。像 Repository、Client、Service 这类对象，通常是无状态的，或者本身就应该长期复用。但有些对象确实不适合放进容器，比如它需要携带请求参数、租户信息，或者只在某次业务处理过程中短暂存在。

这时候更自然的做法，是把工厂注册成 Bean。容器负责管理工厂，运行期对象则由工厂根据业务参数创建。
...
在上面的代码里，`MyServiceFactory` 是一个普通 Bean，它的配置和依赖由 Go-Spring 注入。`MyService` 则不是容器 Bean，它只是由工厂在运行期创建出来的业务对象。

工厂 Bean 不是一种新的 Bean 注册类型。它只是把“长期复用的依赖装配”和“运行期对象创建”分开，容器负责前者，业务工厂负责后者。这样一来，Go-Spring 仍然只需要处理启动期装配，不需要为了少数运行期对象创建场景引入运行期依赖解析和作用域管理这些复杂能力。
