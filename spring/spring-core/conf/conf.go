...
type RedisClientConfig struct {
	...
	ConnectTimeout int    `value:"${redis.connect-timeout:=0}"` // 连接超时，毫秒
	ReadTimeout    int    `value:"${redis.read-timeout:=0}"`    // 读取超时，毫秒
	WriteTimeout   int    `value:"${redis.write-timeout:=0}"`   // 写入超时，毫秒
	IdleTimeout    int    `value:"${redis.idle-timeout:=0}"`    // 空闲连接超时，毫秒
}
