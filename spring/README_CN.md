...
Go-Spring 的配置体系是**严格类型驱动**的：每次配置访问必须声明一个具体的 Go 类型——
简单类型、结构体、map、切片、或已注册的转换器类型。不存在 `any`/`interface{}` 绑定路径；
目标类型决定了 key 如何解析、分层数据如何合并、值如何转换。同样的约束延伸到 `gs.Dync[T]`：
动态刷新复用完全相同的类型检查管道，热加载时校验失败的值会被原子性地拒绝，不会部分更新应用状态。
