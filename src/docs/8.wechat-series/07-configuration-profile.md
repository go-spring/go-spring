...
上一篇咱们讲了配置优先级和合并语义。然后我们知道，命令行参数、环境变量、Profile 配置、基础配置和默认值都可以进入同一个 `path` 空间，并且 Go-Spring 会按照确定的规则得到最终配置。

然而，真实项目里还有一个更具体的问题：配置文件本身应该怎样组织。通常来说，开发、测试、生产等不同环境往往只存在少量差异，比如数据库地址、外部服务超时时间、日志级别和能力开关等，大部分内容都是一样的。如果我们为每个环境都复制一整份配置，那么一旦公共字段需要调整，就要在多份文件里同步完成修改，这样就会带来维护成本。

Go-Spring 提供了 Profile 机制来解决这个问题。Go-Spring 会首先加载基础配置，然后按照当前激活的 Profile 加载和叠加差异配置。这样，我们就可以把公共配置写在基础文件里，把环境差异写在 Profile 文件里。

## 基础配置和 Profile 配置

Go-Spring 将配置文件分成两类。一类是基础配置，文件名是 `app.*`，所有环境都会加载它。另一类是 Profile 配置，文件名是 `app-{profile}.*`，只有对应 Profile 被激活时才会加载。

> `*` 表示任意文件类型，比如 YAML、JSON 等。
> `{profile}` 表示当前激活的 Profile 名称，比如 `prod`、`dev` 等。

如果我们规定在项目中只能使用 YAML 配置文件，那么项目的配置目录通常可以这样组织:
...
`spring.app.config.dir` 和 `spring.profiles.active` 有些不同。通常来说，它的值在所有环境中应当保持一致，只是不同项目可能采用不同的配置目录。因此，我们也可以在代码中为项目设置它的值。
...
Profile 多环境配置的意义是把“公共配置”和“环境差异”分开放置。基础配置负责表达所有环境共享的部分，Profile 配置负责表达当前环境或能力维度的差异。当需要使用多个 Profile 时，按照声明顺序依次叠加就可以了。
